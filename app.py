...
def honor_public():
    ...
    return render_template("honor_public.html", honors=rows)
...
def honor_admin():
    ...
    return render_template("honor_admin.html", honors=rows, certificates=certs, edit_honor=None, today=date.today().isoformat())
...
def honor_edit(hid):
    ...
    certs=db.execute("SELECT id,certificate_no,recipient_name FROM certificates ORDER BY created_at DESC LIMIT 200").fetchall()
    rows=db.execute("SELECT * FROM honor_list ORDER BY honor_date DESC,created_at DESC").fetchall()
    return render_template("honor_admin.html", honors=rows, certificates=certs, edit_honor=h, today=date.today().isoformat())
