...
def members_list():
    ...
    grouped=[]
    for c in committees:
        grouped.append({"name":c["name"],"members":[m for m in members if (m["committee"] or "").strip()==(c["name"] or "").strip()]})
...
def admin_reorder():
    ...
    ids=payload.get("ids")
    table_map={"members":"members","administrators":"administrators","temporary_volunteers":"temporary_volunteers"}
