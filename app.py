...
def admin_dashboard():
    ...
    stats={
        ...
        "admins": db.execute("SELECT COUNT(*) c FROM users WHERE role IN ('ADMIN','SUPER_ADMIN','CREATOR') AND active=1").fetchone()["c"],
...
def _temporary_volunteer_qr(record_id):
    target=url_for("scan_record",kind="temporary",record_id=record_id,_external=True)
    img=qrcode.make(target)
    filename=f"card-temporary-{secure_filename(str(record_id))}.png"
    img.save(os.path.join(UPLOAD_DIR,filename))
    return f"uploads/{filename}"
...
def _report_rows(db, start=None, end=None):
    ...
    equipment_loans = filtered("SELECT l.*, e.name equipment_name, m.name member_name FROM equipment_loans l LEFT JOIN podcast_equipment e ON e.id=l.equipment_id LEFT JOIN members m ON m.id=l.member_id ORDER BY l.issued_at DESC") if _table_exists(db, "equipment_loans") else []
