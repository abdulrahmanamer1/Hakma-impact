...
def init_db():
    ...
    permission_names=[
        ...
        "sections","permissions","backup","system","delete","publish","upload_media","manage_admins","live","certificates","cards","podcast","podcast_admin"
    ]
...
ADMIN_GET_ENDPOINTS = {
    "settings_page", "certificates_admin", "certificate_edit", "certificate_legacy", "cards_admin", "card_print", "audit_log", "backup_export", "admin_new", "admin_edit", "admin_delete",
...
PUBLIC_ENDPOINTS = {"public_home", "public_news", "public_news_detail", "public_page",
                    "committees_list", "initiatives_list", "initiative_view", "members_list", "member_view", "achievements",
                    ...
                    "login", "admin_login", "logout", "static", "signup", "live_room", "live_join", "live_peers_api", "live_signal", "live_leave", "certificate_verify", "podcast_public", "podcast_episode", "card_public"}
...
def live_room(sid):
    ...
    return render_template("live_room.html", live_session=ls, is_broadcaster=is_admin(), source_mode=request.args.get("source") == "1")
...
def live_join(sid):
    ...
    if role in ("broadcaster","source") and not is_admin(): return jsonify({"ok":False,"error":"Admin access required"}),403
...
def task_form(tid=None):
    ...
    if request.method == "POST":
        ...
        if tid:
            ...
            db.execute("""INSERT INTO tasks(id,title,assignee,deadline,priority,status,description,latitude,longitude,map_visible,created_at)
...
def _card_qr(kind, record_id, number):
    target=url_for("card_public",kind=kind,record_id=record_id,_external=True)
...
def certificates_admin():
    ...
    if request.method=="POST":
        ...
        db.commit(); db.execute("UPDATE certificates SET qr_path=? WHERE id=?",(_certificate_qr(cid,no),cid)); db.commit()
...
def live_admin():
    ...
    if request.method=="POST":
        ...
        if mode=="internal":
            ...
            if db.execute("SELECT COUNT(*) c FROM live_sessions WHERE status='live'").fetchone()["c"] > 6:
                db.execute("DELETE FROM live_sessions WHERE id=?",(sid,)); db.commit(); flash("الحد الأقصى 6 بثوث مباشرة", "error"); return redirect(url_for("live_admin"))
...
def _report_ar(value):
    """Shape Arabic text for ReportLab when optional shaping packages exist."""
...
def _report_pdf(path, data, start=None, end=None):
    ...
    doc = SimpleDocTemplate(path, pagesize=page, rightMargin=7*mm, leftMargin=7*mm, topMargin=7*mm, bottomMargin=7*mm,
                            title="HIKMA IMPACT — التقرير الشامل", author="HIKMA IMPACT")
    ...
    story.append(Table([[Paragraph(_report_ar("HIKMA IMPACT"), styles["title"]), Paragraph(_report_ar(title), styles["title"])]], colWidths=[115*mm, 150*mm], rowHeights=[24*mm], style=[
        ("BACKGROUND",(0,0),(-1,-1),navy),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LEFTPADDING",(0,0),(-1,-1),8*mm),("RIGHTPADDING",(0,0),(-1,-1),8*mm)
    ]))
    ...
    def footer(canvas, doc):
        ...
        canvas.drawRightString(page[0]-7*mm,4*mm,_report_ar(f"فريق الحكمة التطوعي · HIKMA IMPACT · صفحة {doc.page}")); canvas.restoreState()
...
def export_complete_pdf():
    ...
    return send_file(buf,mimetype="application/pdf",as_attachment=True,download_name=f"Hikma-Impact-Complete-Report-{date.today().isoformat()}.pdf")
...
def export_complete_xlsx():
    ...
    return send_file(buf,mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",as_attachment=True,download_name=f"Hikma-Impact-Complete-Data-{date.today().isoformat()}.xlsx")
...
def backup_export():
    ...
    tables = [
        ...
        "certificates", "attachments", "card_templates", "podcast_shows", "podcast_guests", "podcast_episodes", "podcast_clips", "podcast_equipment"
    ]
    data = {"version": 14, "created_at": now_iso(), "tables": {}}
    with db:
        for t in tables:
...
def backup_import():
    ...
    try:
        ...
        table_order = [
            ...
            "card_templates", "podcast_shows", "podcast_guests", "podcast_episodes", "podcast_clips", "podcast_equipment",
