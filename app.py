...
PUBLIC_ENDPOINTS = {"public_home", "public_news", "public_news_detail", "public_page",
                    ...
                    "login", "admin_login", "logout", "static", "signup", "academy_learn", "academy_lesson_complete", "academy_quiz_submit", "academy_media", "academy_payment", "academy_my_payments", "live_room", "live_join", "live_peers_api", "live_signal", "live_leave", "certificate_verify", "podcast_public", "podcast_episode", "card_public"}
...
def scan_record(kind, record_id):
    ...
    if kind in ("member","admin"):
        ...
        row=db.execute(f"SELECT * FROM {table} WHERE id=?",(record_id,)).fetchone()
        if not row: return "السجل غير موجود",404
        # If an authorized admin scans the printed team-card QR, register
        # attendance automatically and avoid duplicate check-ins for the same day.
        if is_admin() and kind=="member":
            today=iraq_today()
            existing=db.execute("SELECT id FROM attendance WHERE member_id=? AND date=? ORDER BY start_time DESC LIMIT 1",(record_id,today)).fetchone()
            if not existing:
                aid=uid("att")
                db.execute("INSERT INTO attendance(id,member_id,date,status,initiative_id,start_time,end_time,hours) VALUES(?,?,?,?,?,?,?,?)",
                           (aid,record_id,today,"Present",None,iraq_now_time(),None,0.0))
                db.commit()
                try:
                    add_points(record_id,get_points_config().get("attendance",10),f"حضور QR بتاريخ {today}")
                except Exception:
                    pass
                log_action("QR attendance check-in",row["name"])
                flash(f"تم تسجيل حضور {row['name']} تلقائيًا بالـQR","ok")
                return redirect(url_for("attendance_list"))
        if not row["card_public"] and not is_admin():
            return render_template("403.html",permission="البطاقة خاصة"),403
        return redirect(url_for("card_public",kind=kind,record_id=record_id))
    if kind=="event":
        row=db.execute("SELECT * FROM events WHERE id=?",(record_id,)).fetchone()
        if not row:return "الفعالية غير موجودة",404
        return render_template("scan_action.html",kind=kind,record=row,title="تسجيل الفعالية",action_url=url_for("attendance_new"))
    if kind=="task":
        row=db.execute("SELECT * FROM tasks WHERE id=?",(record_id,)).fetchone()
        if not row:return "المهمة غير موجودة",404
        return render_template("scan_action.html",kind=kind,record=row,title="تسجيل المهمة",action_url=url_for("tasks_list"))
...
def certificate_view(cid):
    if not is_admin(): return redirect(url_for("admin_login"))
    c=get_db().execute("SELECT * FROM certificates WHERE id=? OR certificate_no=?",(cid,cid)).fetchone()
    if not c:
        flash("الشهادة غير موجودة في قاعدة البيانات. تحقق من رقمها أو أعد الإصدار.","error")
        return redirect(url_for("certificates_admin"))
        ...
        if mode=="internal":
            ...
            db.execute("""INSERT INTO live_sessions(
                ...
                (sid,title,desc,initiative_id,"internal","","Internal","live",current_user()["id"],now_iso(),now_iso(),
                 broadcast_type,event_location,scheduled_date,scheduled_time,json.dumps(links,ensure_ascii=False)))
...
def academy_admin():
    ...
    if request.method=="POST":
        ...
        if action=="course":
            ...
            db.execute("INSERT INTO academy_lessons(id,course_id,title,content,video,duration,sort_order,created_at) VALUES(?,?,?,?,?,?,?,?)",(uid("lesson"),request.form.get("course_id"),request.form.get("title"),request.form.get("content"),request.form.get("video"),request.form.get("duration"),int(request.form.get("sort_order") or 0),now_iso()))
        elif action=="quiz":
...
def equipment_admin():
    ...
    return render_template("equipment_admin.html",equipment=db.execute("SELECT * FROM podcast_equipment ORDER BY category,name").fetchall(),loans=db.execute("SELECT l.*,e.name equipment_name,m.name member_name FROM equipment_loans l LEFT JOIN podcast_equipment e ON e.id=l.equipment_id LEFT JOIN members m ON m.id=l.member_id ORDER BY l.issued_at DESC LIMIT 100").fetchall(),members=db.execute("SELECT id,name FROM members ORDER BY name").fetchall())
...
def backup_full_export():
    ...
    manifest={"version":18,"created_at":now_iso(),"team":"AHVT","includes":["database","uploads"]}
...
def backup_export():
    ...
    tables = [
        ...
        "certificates", "attachments", "card_templates", "podcast_shows", "podcast_guests", "podcast_episodes", "podcast_clips", "podcast_equipment", "academy_courses", "academy_lessons", "finance_entries", "equipment_loans", "site_notifications"
    ]
    data = {"version": 18, "created_at": now_iso(), "tables": {}}
    with db:
        for t in tables:
