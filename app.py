...
def init_db():
    ...
    defaults = {
        ...
        "qrPublicEnabled": "1",
        "teamVision": "", "teamMission": "", "teamGoals": "", "teamSlogan": "تطوع .. تعلم .. تطور",
...
def impact_public():
    ...
    return render_template("impact.html", members=members, initiatives=initiatives, hours=round(hours), admins=admins, beneficiaries=beneficiaries, goals=goals, points=points)
...
def admin_form(aid=None):
    ...
    if request.method == "POST":
        ...
        data = (name, request.form.get("position", ""), request.form.get("committee", ""),
                ...
                request.form.get("card_expiry", "") or (a["card_expiry"] if a else ""), request.form.get("nfc_uid", "").strip(), 1 if request.form.get("card_public") else 0, 1 if request.form.get("qr_enabled") else 0)
        if aid:
            db.execute("UPDATE administrators SET name=?,position=?,committee=?,date=?,responsibilities=?,photo=?,department=?,membership_no=?,card_issue_date=?,card_expiry=?,nfc_uid=?,card_public=?,qr_enabled=? WHERE id=?", data + (aid,))
        else:
            db.execute("INSERT INTO administrators(id,name,position,committee,date,responsibilities,photo,department,membership_no,card_issue_date,card_expiry,nfc_uid,card_public,qr_enabled) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (uid("adm"),) + data)
        db.commit()
...
def scan_record(kind, record_id):
    ...
    if kind in ("member","admin","temporary"):
        ...
        return render_template("scan_action.html", kind="person", person_kind=kind, record=row, title="بطاقة QR",
                               action_url=url_for("scan_person_action",kind=kind,record_id=record_id),
...
def settings_page():
    ...
    if request.method == "POST":
        ...
        if form_type == "identity":
            ...
            for key, field in [("siteDescription","site_description"),("teamVision","team_vision"),("teamMission","team_mission"),("teamGoals","team_goals"),("teamSlogan","team_slogan")]:
                set_setting(key, request.form.get(field, ""))
