...
def public_home():
    ...
    return render_template("public_home.html",
        members_count=len(members), admins_count=len(admins), temporary_volunteers_count=temp_count, beneficiaries_count=beneficiaries_count, news_count=news_count, initiatives_count=len(initiatives),
...
def events_admin():
    ...
    if request.method=="POST":
        ...
        db.execute("INSERT INTO events(id,title,date,time,location,description,status,cover_image,created_at,qr_path,latitude,longitude,map_visible) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",(eid,request.form.get("title"),request.form.get("date"),request.form.get("time"),request.form.get("location"),request.form.get("description"),request.form.get("status","upcoming"),cover,now_iso(),_action_qr("event",eid,"action"),lat,lng,1 if request.form.get("map_visible") else 0))
...
def event_edit(eid):
    ...
    if request.method=="POST":
        ...
        db.execute("UPDATE events SET title=?,date=?,time=?,location=?,description=?,status=?,cover_image=?,latitude=?,longitude=?,map_visible=? WHERE id=?",(request.form.get("title"),request.form.get("date"),request.form.get("time"),request.form.get("location"),request.form.get("description"),request.form.get("status","upcoming"),cover,request.form.get("latitude") or None,request.form.get("longitude") or None,1 if request.form.get("map_visible") else 0,eid))
        save_multiple_attachments(request.files.getlist("media_files"), "event", eid, db, request.form.get("title"))
        db.commit(); log_action("Updated event",eid); return redirect(url_for("events_admin"))
    return render_template("events_admin.html",events=db.execute("SELECT * FROM events ORDER BY date DESC").fetchall(),edit_event=e)
...
def card_template_preview(tid):
    ...
    if kind=="temporary":
        ...
        qr=_temporary_volunteer_qr(person["id"])
        return render_template("temporary_volunteer_card.html",volunteer=person,qr_path=qr,printable=True,template=t)
...
def temporary_card_print(record_id):
    ...
    qr=_temporary_volunteer_qr(record_id)
    return render_template("temporary_volunteer_card.html", volunteer=row, qr_path=qr, printable=True)
...
def temporary_card_public(record_id):
    ...
    qr=_temporary_volunteer_qr(record_id)
    return render_template("temporary_volunteer_card.html", volunteer=row, qr_path=qr, printable=False)
