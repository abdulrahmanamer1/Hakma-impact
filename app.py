...
def certificates_admin():
    ...
    if request.method=="POST":
        ...
        db.commit()
        # Generate the QR after the certificate row is committed, then verify
        # that the same connection can immediately read it back. This prevents
        # the old "certificate not found" race/redirect problem.
        ...
        if not created:
            db.rollback()
            flash("تعذر حفظ الشهادة. حاول مرة أخرى.", "error")
            return redirect(url_for("certificates_admin"))
        log_action("Created certificate", f"{recipient} · {no}")
        flash("تم إصدار الشهادة بنجاح", "ok")
        return render_template("certificate.html", certificate=created,
                               team_logo=get_setting("teamLogo", ""),
                               university_logo=get_setting("universityLogo", ""),
                               verify_url=url_for("certificate_verify", certificate_no=created["certificate_no"], _external=True))
...
def certificate_view(cid):
    ...
    if not c:
        flash("الشهادة غير موجودة في سجل الشهادات.", "error")
        return redirect(url_for("certificates_admin"))
...
def chat_send(cid):
    ...
    if f and f.filename:
        ...
        elif ext in {".mp4",".webm",".mov",".m4v"}: path=save_uploaded_video(f,"chat"); media_type="video"
...
def backup_full_export():
    ...
    manifest={"version":21,"created_at":now_iso(),"team":"AHVT","includes":["database","uploads"]}
...
def backup_export():
    ...
    tables = [
        ...
        "certificates", "attachments", "card_templates", "task_completions", "chat_conversations", "chat_messages", "chat_reactions", "podcast_shows", "podcast_guests", "podcast_episodes", "podcast_clips", "podcast_equipment", "academy_courses", "academy_lessons", "finance_entries", "equipment_loans", "site_notifications"
    ]
    data = {"version": 21, "created_at": now_iso(), "tables": {}}
    with db:
        for t in tables:
