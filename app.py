...
ADMIN_GET_ENDPOINTS = {
    "settings_page", "audit_log", "backup_export", "admin_new", "admin_edit", "admin_delete",
    ...
    "partners_admin", "media_admin", "security_page", "certificate", "insights_page", "risk_page", "decision_center",
}
PUBLIC_ENDPOINTS = {"public_home", "public_news", "public_news_detail", "public_page",
                    ...
                    "about_public", "search_public", "volunteer_redirect", "api_impact",
...
def inject_globals():
    return {
        ...
        "site_settings": {k: get_setting(k, "") for k in ["accentColor","navyColor","backgroundColor","fontFamily","heroTitle","heroText","announcement","customCss","teamLogo","universityLogo","favicon","telegramUrl","siteDescription","showPublicAdmins","showPublicNews"]},
...
def public_home():
    ...
    return render_template("public_home.html",
        ...
        committees=committees, upcoming=upcoming, featured=featured, public_admins=public_admins)
...
def impact_public():
    ...
    return render_template("impact.html", members=members, initiatives=initiatives, hours=round(hours), admins=admins, goals=goals)
...
def certificate(mid):
    # HTML print-ready certificate; no private member data is exposed beyond the requested name.
    m=get_db().execute("SELECT * FROM members WHERE id=?",(mid,)).fetchone()
    if not m: return redirect(url_for("members_list"))
    return render_template("certificate.html",member=m,issued=date.today().isoformat(),hours=get_db().execute("SELECT COALESCE(SUM(i.hours),0) h FROM initiative_participants p JOIN initiatives i ON i.id=p.initiative_id WHERE p.member_id=?",(mid,)).fetchone()["h"] or 0)
...
def initiative_view(iid):
    ...
    return render_template("initiative_view.html", i=i, participants=participants, public_view=not is_admin())
...
def backup_import():
    ...
    try:
        ...
        table_cols = {
            ...
            "media": ["id","title","url","category","initiative_id","public","created_at"],
