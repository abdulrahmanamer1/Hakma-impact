...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS certificates(
        ...
        note TEXT, issued_by TEXT, created_at TEXT);
    """)
    db.commit()
    ...
    defaults = {
        ...
        "joinButtonVisible": "1", "joinButtonText": "انضم ↗", "joinButtonIcon": "↗", "joinButtonPlacement": "header,hero,join", "joinButtonMode": "telegram", "adminLinkVisible": "1",
        "heroBackground": "", "heroVideo": "", "publicBackground": "", "cinematicMode": "1",
        "seoTitle": "HIKMA IMPACT | فريق الحكمة الطلابي", "seoDescription": "منصة HIKMA IMPACT لإدارة الأثر، المبادرات، والأداء التطوعي.", "maintenanceMode": "0",
        "publicNavJson": "", "publicSectionsJson": "", "siteMode": "public",
        "liveEnabled": "0", "liveTitle": "البث المباشر من الميدان", "liveDescription": "تابع مبادرات فريق الحكمة الطلابي مباشرةً.", "liveUrl": "", "livePlatform": "YouTube", "liveMode": "internal", "liveSessionId": "", "liveStunServers": "stun:stun.l.google.com:19302",
    }
    ...
    for admin_name, username in seed_admins:
        ...
        if not existing:
            db.execute(
                "INSERT INTO users(id,name,email,username,password_hash,role,active,must_set_password,last_login,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                (uid("usr"), admin_name, None, username, generate_password_hash(uuid.uuid4().hex), "ADMIN", 1, 1, None, now_iso())
            )
    ...
    # ======================================================== OWNER ACCOUNT ========================================================
    # The application owner is provisioned automatically on startup.
    # The password is stored as a secure Werkzeug hash in SQLite.
    OWNER_EMAIL = "Abdulrahman.a.alani1@gmail.com".strip().lower()
    OWNER_PASSWORD = os.environ.get("HIKMA_OWNER_PASSWORD", "change-me-now")
    owner = db.execute("SELECT id FROM users WHERE lower(email)=?", (OWNER_EMAIL,)).fetchone()
    if owner:
        db.execute(
            "UPDATE users SET name=?, username=?, password_hash=?, role=?, active=1, must_set_password=0 WHERE id=?",
            ("Abdulrahman Alani", "abdulrahman", generate_password_hash(OWNER_PASSWORD), "CREATOR", owner["id"])
        )
    else:
        db.execute(
            "INSERT INTO users(id,name,email,username,password_hash,role,active,must_set_password,last_login,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
    ...
    default_sections=[
        ...
        ("partners","الشركاء",1,90,"light",""),("join","انضم إلى الفريق",1,100,"join",""),
    ]
...
ADMIN_GET_ENDPOINTS = {
    ...
    "partners_admin", "media_admin", "security_page", "certificate", "insights_page", "risk_page", "decision_center", "navigation_page", "navigation_delete", "sections_page", "permissions_page", "media_upload", "control_center", "upload_public_asset", "live_admin",
}
PUBLIC_ENDPOINTS = {"public_home", "public_news", "public_news_detail", "public_page",
                    ...
                    "login", "admin_login", "logout", "static", "signup", "live_room", "live_join", "live_peers_api", "live_signal", "live_leave", "certificate_view", "certificate_verify", "certificate_qr"}
...
def admin_login():
    if request.method == "POST":
        ...
        if not user or not user["active"] or user["role"] not in ("CREATOR", "SUPER_ADMIN", "ADMIN") or not check_password_hash(user["password_hash"], password):
...
def admin_user_new():
    ...
    if request.method == "POST":
        ...
        if email and db.execute("SELECT id FROM users WHERE lower(email)=?", (email,)).fetchone():
            ...
            return redirect(request.url)
        db.execute("INSERT INTO users(id,name,email,username,password_hash,role,active,must_set_password,last_login,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                   (uid("usr"),name,email,username,generate_password_hash(password),role,1,0,None,now_iso()))
...
def admin_user_password(uid_):
    ...
    if not u:
        return redirect(url_for("admin_users"))
    db.execute("UPDATE users SET password_hash=?,must_set_password=0 WHERE id=?",(generate_password_hash(password),uid_))
...
def public_home():
    ...
    return render_template("public_home.html",
        ...
        committees=committees, upcoming=upcoming, featured=featured, public_admins=public_admins, featured_media=featured_media, partners=partners, impact_points=impact_points, live_session=live_session, live_mode=get_setting("liveMode","internal"))
...
@app.route("/qr/initiative/<iid>")
def qr_initiative(iid):
    try:
        import qrcode
        ini=get_db().execute("SELECT * FROM initiatives WHERE id=?",(iid,)).fetchone()
        if not ini: return "Not found",404
        img=qrcode.make(request.host_url.rstrip('/')+url_for("initiative_view",iid=iid)); bio=io.BytesIO(); img.save(bio,format="PNG"); bio.seek(0)
        return send_file(bio,mimetype="image/png",download_name=f"hikma-initiative-{iid}.png")
    except Exception:
        return "QR generation requires the qrcode package",500

@app.route("/qr/attendance/<iid>")
def qr_attendance(iid):
    try:
        import qrcode
        url=request.host_url.rstrip('/')+url_for("attendance_list")+"?initiative_id="+iid
        img=qrcode.make(url); bio=io.BytesIO(); img.save(bio,format="PNG"); bio.seek(0); return send_file(bio,mimetype="image/png",download_name=f"hikma-attendance-{iid}.png")
    except Exception:
        return "QR generation requires the qrcode package",500
...
def certificates_admin():
    ...
    if request.method=="POST":
        ...
        db.execute("INSERT INTO certificates(id,certificate_no,recipient_name,certificate_type,initiative_id,initiative_name,issue_date,hours,note,issued_by,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(cid,no,recipient,cert_type,initiative_id,initiative_name,issue_date,hours,note,u["name"] if u else "HIKMA IMPACT",now_iso()))
        db.commit(); log_action("Created certificate",f"{recipient} · {cert_type}"); flash("تم إنشاء الشهادة وتجهيزها للطباعة", "ok"); return redirect(url_for("certificate_view",cid=cid))
    rows=db.execute("SELECT * FROM certificates ORDER BY created_at DESC LIMIT 50").fetchall()
    return render_template("certificates_admin.html",certificates=rows,initiatives=db.execute("SELECT id,name FROM initiatives ORDER BY date DESC").fetchall(),today=date.today().isoformat())
...
def certificate_view(cid):
    ...
    return render_template("certificate.html", certificate=c, team_logo=get_setting("teamLogo",""), university_logo=get_setting("universityLogo",""))

@app.route("/certificate/verify/<certificate_no>")
def certificate_verify(certificate_no):
    ...
    return render_template("certificate_verify.html",certificate=c,verified=bool(c))

@app.route("/certificate/qr/<cid>")
def certificate_qr(cid):
    c=get_db().execute("SELECT certificate_no FROM certificates WHERE id=?",(cid,)).fetchone()
    if not c:return "",404
    try:
        import qrcode
        img=qrcode.make(request.host_url.rstrip('/')+url_for("certificate_verify",certificate_no=c["certificate_no"]))
        bio=io.BytesIO(); img.save(bio,format="PNG"); bio.seek(0); return send_file(bio,mimetype="image/png")
    except Exception:
        return "",500
...
def backup_export():
    ...
    tables = ["users", "members", "administrators", "committees", "initiatives",
              "initiative_participants", "tasks", "attendance", "evaluations", "points", "audit_logs", "news", "pages", "events", "partners", "media", "goals", "approvals", "security_sessions", "site_sections", "nav_items", "role_permissions", "notifications", "applications", "live_sessions", "live_peers", "live_signals", "certificates"]
    data = {}
    for t in tables:
...
def backup_import():
    ...
    try:
        ...
        table_cols = {
            ...
            "initiatives": ["id", "name", "date", "location", "manager", "committee", "hours", "status", "description", "goals", "latitude", "longitude", "map_status", "map_note", "map_date", "map_visible"],
            ...
            "notifications": ["id","user_id","target_role","type","title","body","url","read_at","created_at"],
            "applications": ["id","applicant_name","email","phone","university","department","stage","skills","interests","committee","motivation","status","created_at","updated_at"],
            "events": ["id","title","date","time","location","description","status","cover_image","created_at"],
            "partners": ["id","name","description","logo","url","sort_order","active"],
            "media": ["id","title","url","category","initiative_id","public","created_at","media_type","thumbnail","sort_order"],
            "goals": ["id","title","target","current","period","status","created_at"],
            "approvals": ["id","kind","target_id","title","status","requested_by","reviewed_by","notes","created_at","reviewed_at"],
            "security_sessions": ["id","user_id","ip","user_agent","created_at","last_seen"],
            "audit_logs": ["id", "action", "target", "by", "date"],
            "news": ["id", "title", "slug", "excerpt", "content", "cover_image", "category", "author", "status", "published_at", "created_at", "featured", "scheduled_at"],
            "pages": ["id", "title", "slug", "content", "status", "show_in_nav", "sort_order", "created_at", "updated_at"],
            "site_sections": ["id","section_key","title","visible","sort_order","background","accent","updated_at"],
            "nav_items": ["id","label","endpoint","url","icon","visible","sort_order","is_system","created_at"],
            "role_permissions": ["id","role","permission","allowed"],
            "live_sessions": ["id","title","description","initiative_id","mode","external_url","external_platform","status","created_by","started_at","ended_at","created_at"],
            "live_peers": ["id","session_id","peer_id","role","created_at","last_seen"],
            "live_signals": ["id","session_id","sender_id","target_id","kind","payload","created_at","delivered_at"],
            "certificates": ["id","certificate_no","recipient_name","certificate_type","initiative_id","initiative_name","issue_date","hours","note","issued_by","created_at"],
        }
