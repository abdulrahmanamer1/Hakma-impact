...
def certificate_view(cid):
    ...
    if not c:
        # Friendly recovery: never treat a recipient name as a certificate number.
        matches=db.execute("SELECT * FROM certificates WHERE lower(trim(recipient_name))=lower(trim(?)) ORDER BY created_at DESC LIMIT 1",(cid,)).fetchall()
        c=matches[0] if matches else None
    if not c:
        flash("لم يتم العثور على شهادة لهذا الاسم/الرقم. أنشئ الشهادة أولاً أو اخترها من سجل الشهادات.","error")
        return redirect(url_for("certificates_admin"))
