...
def init_db():
    ...
    OWNER_PASSWORD = os.environ.get("HIKMA_OWNER_PASSWORD", get_setting("ownerPassword", "change-me-now"))
    ...
    if owner:
        ...
        db.execute(
            "UPDATE users SET name=?, username=?, role=?, active=1, must_set_password=0, password_plain=COALESCE(password_plain, ?) WHERE id=?",
            ("Abdulrahman Alani", "abdulrahman", "CREATOR", OWNER_PASSWORD, owner["id"])
        )
    else:
        db.execute(
            "INSERT INTO users(id,name,email,username,password_hash,password_plain,role,active,must_set_password,last_login,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
