...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS news_images(
        id TEXT PRIMARY KEY, news_id TEXT NOT NULL, image_path TEXT NOT NULL,
...
def public_news():
    ...
    news_data=[]
    for n in rows:
        news_data.append({"n": n, "images": news_image_paths(n["id"])})
...
def public_news_detail(slug):
    ...
    return render_template("news_detail.html", n=n, images=news_image_paths(n["id"]))
...
def news_new():
    if request.method=="POST":
        ...
        for f in request.files.getlist("images"):
            path=save_uploaded_image(f, "news")
            if path: uploaded.append(path)
        cover = uploaded[0] if uploaded else request.form.get("cover_image","").strip()
        db.execute("INSERT INTO news(id,title,slug,excerpt,content,cover_image,category,author,status,published_at,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                   (uid("news"),title,slug,request.form.get("excerpt",""),request.form.get("content",""),cover,request.form.get("category","عام"),author,status,now_iso() if status=="published" else None,now_iso()))
        nid=db.execute("SELECT id FROM news WHERE slug=?", (slug,)).fetchone()["id"]
        for idx,path in enumerate(uploaded):
            db.execute("INSERT INTO news_images(id,news_id,image_path,sort_order) VALUES(?,?,?,?)", (uid("nimg"),nid,path,idx))
...
def news_edit(nid):
    ...
    if request.method=="POST":
        ...
        for f in request.files.getlist("images"):
            path=save_uploaded_image(f, "news")
            if path: uploaded.append(path)
        existing=news_image_paths(nid)
        all_images=existing + uploaded
        cover = all_images[0] if all_images else request.form.get("cover_image","").strip()
        ...
        for idx,path in enumerate(uploaded, start=len(existing)):
            db.execute("INSERT INTO news_images(id,news_id,image_path,sort_order) VALUES(?,?,?,?)", (uid("nimg"),nid,path,idx))
        db.commit(); log_action("Updated news",n["title"]); flash("تم تحديث الخبر", "ok"); return redirect(url_for("news_admin"))
    return render_template("news_form.html", n=n, images=news_image_paths(nid))

@app.route("/admin/news/<nid>/delete", methods=["POST"])
def news_delete(nid):
    db=get_db()
    db.execute("DELETE FROM news_images WHERE news_id=?", (nid,))
    db.execute("DELETE FROM news WHERE id=?",(nid,))
    db.commit(); log_action("Deleted news",nid); return redirect(url_for("news_admin"))
...
def member_form(mid=None):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            return redirect(request.url)
        photo = request.form.get("photo", "") or (member["photo"] if member else "")
        uploaded_photo = save_uploaded_image(request.files.get("photo_file"), f"member_{mid or 'new'}")
        if uploaded_photo:
...
def save_uploaded_image(file_obj, prefix="img"):
...
def news_image_paths(news_id):
    db = get_db()
    rows = db.execute("SELECT image_path FROM news_images WHERE news_id=? ORDER BY sort_order,id", (news_id,)).fetchall()
...
def admin_form(aid=None):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            return redirect(request.url)
        photo = request.form.get("photo", "") or (a["photo"] if a and "photo" in a.keys() else "")
        uploaded_photo = save_uploaded_image(request.files.get("photo_file"), f"admin_{aid or 'new'}")
        if uploaded_photo:
            photo = uploaded_photo
...
def backup_import():
    ...
    try:
        ...
        table_cols = {
            ...
            "administrators": ["id", "name", "position", "committee", "date", "responsibilities", "photo"],
            ...
            "news_images": ["id", "news_id", "image_path", "sort_order"],
