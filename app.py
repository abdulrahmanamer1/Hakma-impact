...
def temporary_volunteers_admin():
    ...
    if request.method=="POST":
        ...
        vid=uid("tv")
        db.execute("""INSERT INTO temporary_volunteers(id,name,phone,department,stage,specialty,task,committee,start_date,end_date,hours,benefits,status,photo,card_public,qr_enabled,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(vid,request.form.get("name","").strip(),request.form.get("phone",""),request.form.get("department",""),request.form.get("stage",""),request.form.get("specialty",""),request.form.get("task",""),request.form.get("committee",""),request.form.get("start_date") or None,request.form.get("end_date") or None,float(request.form.get("hours",0) or 0),request.form.get("benefits",""),"active",photo,1 if request.form.get("card_public") else 0,1 if request.form.get("qr_enabled") else 0,now_iso()))
...
def members_list():
    ...
    grouped=[]
    for c in committees:
        grouped.append({"id":c["id"],"name":c["name"],"members":[m for m in members if (m["committee"] or "").strip()==(c["name"] or "").strip()]})
    assigned={m["id"] for g in grouped for m in g["members"]}
    unassigned=[m for m in members if m["id"] not in assigned]
    if unassigned: grouped.append({"name":"أعضاء بدون لجنة", "members":unassigned})
