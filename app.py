...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS news_images(
        id TEXT PRIMARY KEY, news_id TEXT, path TEXT, sort_order INTEGER DEFAULT 0, created_at TEXT);
    ...
    CREATE TABLE IF NOT EXISTS initiative_images(
        id TEXT PRIMARY KEY, initiative_id TEXT, path TEXT, sort_order INTEGER DEFAULT 0, created_at TEXT);
...
def save_uploaded_image(file_storage, prefix="img"):
    ...
    if ext not in ALLOWED_IMAGE_EXTS:
        return None
    filename = f"{prefix}_{uuid.uuid4().hex[:14]}{ext}"
    file_storage.save(os.path.join(UPLOAD_DIR, filename))
    return f"uploads/{filename}"

def save_uploaded_images(files, prefix="img"):
    paths = []
    for f in files or []:
        p = save_uploaded_image(f, prefix)
        if p:
            paths.append(p)
    return paths
...
def inject_globals():
    return {
        ...
        "site_settings": {k: get_setting(k, "") for k in ["accentColor","navyColor","backgroundColor","fontFamily","heroTitle","heroText","announcement","customCss","teamLogo","universityLogo","favicon"]},
...
def admin_login():
    if request.method == "POST":
        ...
        get_db().execute("UPDATE users SET last_login=? WHERE id=?", (now_iso(), user["id"]))
        get_db().commit()
...
def public_news_detail(slug):
    ...
    images=get_db().execute("SELECT * FROM news_images WHERE news_id=? ORDER BY sort_order",(n["id"],)).fetchall()
    return render_template("news_detail.html", n=n, images=images)
...
def news_new():
    if request.method=="POST":
        ...
        cover=save_uploaded_image(request.files.get("cover_file"), "news_cover") or request.form.get("cover_image","")
        nid=uid("news")
        db.execute("INSERT INTO news(id,title,slug,excerpt,content,cover_image,category,author,status,published_at,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                   (nid,title,slug,request.form.get("excerpt",""),request.form.get("content",""),cover,request.form.get("category","عام"),author,status,now_iso() if status=="published" else None,now_iso()))
        for idx,path in enumerate(save_uploaded_images(request.files.getlist("gallery"), "news")):
            db.execute("INSERT INTO news_images(id,news_id,path,sort_order,created_at) VALUES(?,?,?,?,?)", (uid("nimg"),nid,path,idx,now_iso()))
        db.commit(); log_action("Created news",title); flash("تم حفظ الخبر", "ok"); return redirect(url_for("news_admin"))
    return render_template("news_form.html", n=None, images=[])
...
def news_edit(nid):
    ...
    if request.method=="POST":
        status=request.form.get("status","draft")
        cover=save_uploaded_image(request.files.get("cover_file"), "news_cover") or request.form.get("cover_image", n["cover_image"] or "")
        db.execute("UPDATE news SET title=?,slug=?,excerpt=?,content=?,cover_image=?,category=?,status=?,published_at=? WHERE id=?",
                   (request.form.get("title",""),request.form.get("slug",""),request.form.get("excerpt",""),request.form.get("content",""),cover,request.form.get("category","عام"),status,now_iso() if status=="published" else None,nid))
        paths=save_uploaded_images(request.files.getlist("gallery"), "news")
        start=db.execute("SELECT COALESCE(MAX(sort_order),-1)+1 n FROM news_images WHERE news_id=?",(nid,)).fetchone()["n"]
        for idx,path in enumerate(paths): db.execute("INSERT INTO news_images(id,news_id,path,sort_order,created_at) VALUES(?,?,?,?,?)", (uid("nimg"),nid,path,start+idx,now_iso()))
        db.commit(); log_action("Updated news",n["title"]); flash("تم تحديث الخبر", "ok"); return redirect(url_for("news_admin"))
    images=db.execute("SELECT * FROM news_images WHERE news_id=? ORDER BY sort_order",(nid,)).fetchall()
    return render_template("news_form.html", n=n, images=images)

@app.route("/admin/news/<nid>/delete", methods=["POST"])
def news_delete(nid):
    db=get_db(); db.execute("DELETE FROM news_images WHERE news_id=?",(nid,)); db.execute("DELETE FROM news WHERE id=?",(nid,)); db.commit(); log_action("Deleted news",nid); return redirect(url_for("news_admin"))
...
def member_form(mid=None):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            return redirect(request.url)
        photo = save_uploaded_image(request.files.get("photo_file"), "member") or request.form.get("photo", "")
...
@app.route("/administrators/<aid>/photo", methods=["POST"])
def admin_photo_upload(aid):
    if not is_admin():
        return redirect(url_for("admin_login"))
    f=request.files.get("photo_file")
    path=save_uploaded_image(f, f"admin_{aid}")
    if not path:
        flash("اختر صورة صحيحة", "error")
        return redirect(url_for("admins_list"))
    db=get_db(); db.execute("UPDATE administrators SET photo=? WHERE id=?", (path, aid)); db.commit()
    log_action("Updated administrator photo", aid)
    flash("تم تحديث الصورة الشخصية", "ok")
    return redirect(url_for("admins_list"))
...
def admin_form(aid=None):
    ...
    if request.method == "POST":
        ...
        data = (name, request.form.get("position", ""), request.form.get("committee", ""),
                request.form.get("date", ""), request.form.get("responsibilities", ""), a["photo"] if a and "photo" in a.keys() else "")
        uploaded_photo=save_uploaded_image(request.files.get("photo_file"), "admin")
        if uploaded_photo:
            data=data[:-1]+(uploaded_photo,)
        if aid:
            db.execute("""UPDATE administrators SET name=?,position=?,committee=?,date=?,responsibilities=?,photo=?
                           WHERE id=?""", data + (aid,))
        else:
            db.execute("""INSERT INTO administrators(id,name,position,committee,date,responsibilities,photo)
                           VALUES(?,?,?,?,?,?,?)""", (uid("adm"),) + data)
        db.commit()
...
def initiatives_list():
    ...
    data = [{"i": i, "pcount": initiative_participant_count(db, i["id"]), "cover": (db.execute("SELECT path FROM initiative_images WHERE initiative_id=? ORDER BY sort_order LIMIT 1", (i["id"],)).fetchone() or {"path": ""})["path"]} for i in initiatives]
...
def initiative_form(iid=None):
    ...
    if request.method == "POST":
        ...
        db.commit()
        target_iid = iid or db.execute("SELECT id FROM initiatives WHERE name=? ORDER BY rowid DESC LIMIT 1", (name,)).fetchone()["id"]
        paths=save_uploaded_images(request.files.getlist("gallery"), "initiative")
        start=db.execute("SELECT COALESCE(MAX(sort_order),-1)+1 n FROM initiative_images WHERE initiative_id=?",(target_iid,)).fetchone()["n"]
        for idx,path in enumerate(paths): db.execute("INSERT INTO initiative_images(id,initiative_id,path,sort_order,created_at) VALUES(?,?,?,?,?)", (uid("iimg"),target_iid,path,start+idx,now_iso()))
        db.commit()
        flash("تم الحفظ بنجاح", "ok")
        return redirect(url_for("initiatives_list"))
    images=db.execute("SELECT * FROM initiative_images WHERE initiative_id=? ORDER BY sort_order",(iid,)).fetchall() if iid else []
    return render_template("initiative_form.html", i=i, committees=committees, members=members, images=images)


@app.route("/initiatives/<iid>/delete", methods=["POST"])
def initiative_delete(iid):
    db = get_db()
    db.execute("DELETE FROM initiative_images WHERE initiative_id=?", (iid,))
...
def initiative_view(iid):
    ...
    images=db.execute("SELECT * FROM initiative_images WHERE initiative_id=? ORDER BY sort_order",(iid,)).fetchall()
    participants = []
    for pid in participant_ids:
        m = db.execute("SELECT * FROM members WHERE id=?", (pid,)).fetchone()
        if m:
            participants.append(m)
    return render_template("initiative_view.html", i=i, participants=participants, images=images)
...
def backup_export():
    ...
    tables = ["users", "members", "administrators", "committees", "initiatives",
              "initiative_participants", "initiative_images", "tasks", "attendance", "evaluations", "points", "audit_logs", "news", "news_images", "pages"]
    data = {}
    for t in tables:
