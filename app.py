...
ADMIN_GET_ENDPOINTS = {
    "settings_page", "certificates_admin", "certificate_edit", "certificate_legacy", "card_view", "audit_log", "backup_export", "admin_new", "admin_edit", "admin_delete",
...
PUBLIC_ENDPOINTS = {"public_home", "public_news", "public_news_detail", "public_page",
                    ...
                    "login", "admin_login", "logout", "static", "signup", "card_view", "live_room", "live_join", "live_peers_api", "live_signal", "live_leave", "certificate_verify"}
...
def live_public():
    ...
    active_sessions=db.execute("SELECT ls.*,i.name initiative_name FROM live_sessions ls LEFT JOIN initiatives i ON i.id=ls.initiative_id WHERE ls.status='live' ORDER BY ls.started_at DESC LIMIT 3").fetchall()
...
def events_admin():
    ...
    if request.method=="POST":
        ...
        db.execute("INSERT INTO events(id,title,date,time,location,description,status,cover_image,created_at) VALUES(?,?,?,?,?,?,?,?,?)",(eid,request.form.get("title"),request.form.get("date"),request.form.get("time"),request.form.get("location"),request.form.get("description"),request.form.get("status","upcoming"),cover,now_iso()))
...
def member_form(mid=None):
    ...
    if request.method == "POST":
        ...
        data = (name, request.form.get("email", ""), request.form.get("phone", ""),
                ...
                request.form.get("card_expiry", "") or (member["card_expiry"] if member else ""), request.form.get("nfc_uid", "").strip())
        if mid:
            db.execute("UPDATE members SET name=?,email=?,phone=?,committee=?,position=?,join_date=?,status=?,notes=?,photo=?,department=?,stage=?,membership_no=?,card_issue_date=?,card_expiry=?,nfc_uid=? WHERE id=?", data + (mid,))
            log_action("Updated", f"عضو: {name}")
        else:
            db.execute("INSERT INTO members(id,name,email,phone,committee,position,join_date,status,notes,photo,department,stage,membership_no,card_issue_date,card_expiry,nfc_uid,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (uid("mem"),) + data + (now_iso(),))
...
def admin_form(aid=None):
    ...
    if request.method == "POST":
        ...
        data = (name, request.form.get("position", ""), request.form.get("committee", ""),
                ...
                request.form.get("card_expiry", "") or (a["card_expiry"] if a else ""), request.form.get("nfc_uid", "").strip())
        if aid:
            db.execute("UPDATE administrators SET name=?,position=?,committee=?,date=?,responsibilities=?,photo=?,department=?,membership_no=?,card_issue_date=?,card_expiry=?,nfc_uid=? WHERE id=?", data + (aid,))
        else:
            db.execute("INSERT INTO administrators(id,name,position,committee,date,responsibilities,photo,department,membership_no,card_issue_date,card_expiry,nfc_uid) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", (uid("adm"),) + data)
        db.commit()
...
def _card_qr(kind, record_id, number):
    target=url_for("card_view",kind=kind,record_id=record_id,_external=True)
    img=qrcode.make(target); filename=f"card-{kind}-{secure_filename(str(number)) or uuid.uuid4().hex[:8]}.png"; img.save(os.path.join(UPLOAD_DIR,filename)); return f"uploads/{filename}"

@app.route("/admin/cards/<kind>/<record_id>")
def card_view(kind, record_id):
    ...
    qr=_card_qr(kind,record_id,number); nfc_url=url_for("card_view",kind=kind,record_id=record_id,_external=True)
    return render_template("membership_card.html", person=row, kind=kind, qr_path=qr, nfc_url=nfc_url, team_name=get_setting("teamName","فريق الحكمة التطوعي"))
...
def certificates_admin():
    ...
    if request.method=="POST":
        ...
        sql="INSERT INTO certificates(id,certificate_no,recipient_name,certificate_type,initiative_id,initiative_name,issue_date,hours,note,issued_by,created_at,status,verify_token,template,recipient_member_id,issuer_name,custom_title,custom_intro,custom_body,custom_footer,logo1,logo2,logo3) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        vals=(cid,no,recipient,request.form.get("certificate_type","شهادة مشاركة"),initiative_id,ini["name"] if ini else request.form.get("initiative_name","").strip(),issue_date,hours,request.form.get("note","").strip(),u["name"] if u else issuer,now_iso(),"valid",token,template,request.form.get("recipient_member_id") or None,issuer,request.form.get("custom_title","").strip(),request.form.get("custom_intro","").strip(),request.form.get("custom_body","").strip(),request.form.get("custom_footer","").strip(),_certificate_logo_upload(request.files.get("logo1"),"cert-logo1"),_certificate_logo_upload(request.files.get("logo2"),"cert-logo2"),_certificate_logo_upload(request.files.get("logo3"),"cert-logo3"))
        db.execute(sql,vals); db.commit(); db.execute("UPDATE certificates SET qr_path=? WHERE id=?",(_certificate_qr(cid,no),cid)); db.commit()
        log_action("Created certificate",f"{recipient} · {no}"); flash("تم إصدار الشهادة","ok"); return redirect(url_for("certificate_view",cid=cid))
...
def certificate_reissue(cid):
    ...
    sql="INSERT INTO certificates(id,certificate_no,recipient_name,certificate_type,initiative_id,initiative_name,issue_date,hours,note,issued_by,created_at,status,verify_token,template,recipient_member_id,issuer_name,custom_title,custom_intro,custom_body,custom_footer,logo1,logo2,logo3) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
    vals=(new_id,no,c["recipient_name"],c["certificate_type"],c["initiative_id"],c["initiative_name"],date.today().isoformat(),c["hours"],c["note"],current_user()["name"],now_iso(),"valid",token,c["template"],c["recipient_member_id"],c["issuer_name"],c["custom_title"],c["custom_intro"],c["custom_body"],c["custom_footer"],c["logo1"],c["logo2"],c["logo3"])
...
def media_upload():
    ...
    for f in files:
        ...
        elif ext in {".mp4",".webm",".mov",".m4v"}: path=save_uploaded_video(f,"media"); mtype="video"
        else: continue
...
def live_admin():
    ...
    if request.method=="POST":
        ...
        if mode=="internal":
            sid=uid("live")
            db.execute("INSERT INTO live_sessions(id,title,description,initiative_id,mode,external_url,external_platform,status,created_by,started_at,created_at,channel_name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",(sid,title,desc,initiative_id,"internal","","Internal","live",current_user()["id"],now_iso(),now_iso(),request.form.get("channel_name","").strip()))
            if db.execute("SELECT COUNT(*) c FROM live_sessions WHERE status='live'").fetchone()["c"] >= 3:
                db.execute("DELETE FROM live_sessions WHERE id=?",(sid,)); db.commit(); flash("الحد الأقصى 3 بثوث مباشرة", "error"); return redirect(url_for("live_admin"))
    ...
    return render_template("live_admin.html",active_sessions=active_sessions,active_session=(active_sessions[0] if active_sessions else None),sessions=sessions,initiatives=db.execute("SELECT id,name FROM initiatives ORDER BY date DESC").fetchall())
