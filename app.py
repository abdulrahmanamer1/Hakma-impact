...
def _normalise_asset_path(path):
    if path is None:
        return ""
...
def asset_url(path):
    """Resolve static files and persistent uploads consistently across every template."""
    path=_normalise_asset_path(path)
    if not path:
        return ""
    if path.startswith(("http://","https://","data:","blob:","/static/")):
        return path
    ...
    if path.startswith("uploads/"):
        ...
        if not os.path.isfile(os.path.join(UPLOAD_DIR, filename)):
            return url_for("asset_placeholder")
...
def asset_placeholder():
    svg="""<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 800 520\"><rect width=\"800\" height=\"520\" fill=\"#071522\"/><circle cx=\"400\" cy=\"220\" r=\"82\" fill=\"#0b2b45\"/><path d=\"M350 245l35-42 28 30 24-27 53 55H310z\" fill=\"#1976d2\" opacity=\".9\"/><text x=\"400\" y=\"370\" fill=\"#9fb3c5\" font-family=\"Arial\" font-size=\"28\" text-anchor=\"middle\">AHVT</text><text x=\"400\" y=\"408\" fill=\"#647b8f\" font-family=\"Arial\" font-size=\"18\" text-anchor=\"middle\">الصورة غير متوفرة</text></svg>"""
    return app.response_class(svg, mimetype="image/svg+xml", headers={"Cache-Control":"no-store"})
...
def save_image_as_jpg(file_storage, prefix="img"):
    ...
    try:
        ...
        if has_alpha:
            ...
            name = f"{prefix}-{uuid.uuid4().hex[:12]}.png"
            img.save(os.path.join(UPLOAD_DIR, name), "PNG", optimize=True)
        else:
            img = img.convert("RGB")
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS external_people(
        id TEXT PRIMARY KEY, name TEXT NOT NULL, organization TEXT DEFAULT '', role TEXT DEFAULT '',
        phone TEXT DEFAULT '', email TEXT DEFAULT '', photo TEXT DEFAULT '', notes TEXT DEFAULT '',
    ...
    ensure_column("members", "sort_order", "INTEGER DEFAULT 0")
    ensure_column("committees", "sort_order", "INTEGER DEFAULT 0")
    ensure_column("administrators", "sort_order", "INTEGER DEFAULT 0")
    ensure_column("temporary_volunteers", "sort_order", "INTEGER DEFAULT 0")
    ensure_column("temporary_volunteers", "initiative_id", "TEXT")
    ...
    ensure_column("members", "qr_enabled", "INTEGER DEFAULT 1")
    # Removed legacy modules: strategic goals and approvals.
    db.execute("DROP TABLE IF EXISTS goals")
    db.execute("DROP TABLE IF EXISTS approvals")
    ensure_column("administrators", "qr_enabled", "INTEGER DEFAULT 1")
    ensure_column("temporary_volunteers", "qr_enabled", "INTEGER DEFAULT 1")
    ensure_column("tasks", "assignee_type", "TEXT DEFAULT 'member'")
    ensure_column("tasks", "assignee_id", "TEXT")
    ensure_column("events", "responsible", "TEXT")
    ensure_column("events", "responsible_type", "TEXT DEFAULT 'member'")
    ensure_column("events", "responsible_id", "TEXT")
    db.execute("UPDATE members SET qr_enabled=1 WHERE qr_enabled IS NULL")
    db.execute("UPDATE administrators SET qr_enabled=1 WHERE qr_enabled IS NULL")
    db.execute("UPDATE temporary_volunteers SET qr_enabled=1 WHERE qr_enabled IS NULL")
    ensure_column("administrators", "public_profile", "INTEGER DEFAULT 1")
    ensure_column("events", "qr_path", "TEXT")
    ensure_column("events", "latitude", "REAL")
    ensure_column("events", "longitude", "REAL")
    ensure_column("events", "map_visible", "INTEGER DEFAULT 0")
    ...
    ensure_column("administrators", "positions_json", "TEXT DEFAULT '[]'")
    ensure_column("certificates", "recipient_type", "TEXT DEFAULT 'member'")
    ensure_column("certificates", "recipient_person_id", "TEXT")
    ...
    ensure_column("certificates", "design_positions", "TEXT DEFAULT '{}'")
    ...
    ensure_column("attendance", "person_type", "TEXT DEFAULT 'member'")
    ensure_column("attendance", "person_id", "TEXT")
    ensure_column("attendance", "event_id", "TEXT")
    ensure_column("equipment_loans", "person_type", "TEXT DEFAULT 'member'")
    ensure_column("equipment_loans", "person_id", "TEXT")
    ensure_column("task_completions", "person_type", "TEXT DEFAULT 'member'")
    ensure_column("task_completions", "person_id", "TEXT")
    ...
    defaults = {
        ...
        "qrPublicEnabled": "1",
        "teamVision": "", "teamMission": "", "teamSlogan": "تطوع .. تعلم .. تطور",
        "liveRecordingEnabled": "1",
    }
    ...
    permission_names=[
        ...
        "achievements","news","events","media","partners","pages","applications","reports","analytics",
        "academy","academy_manage","calendar","media_center","equipment","finance","assistant","site_notifications",
        "notifications","audit","security","insights","risk","appearance","navigation",
        "sections","permissions","backup","system","delete","publish","upload_media","manage_admins","live","live_recordings","certificates","honor","cards","podcast","podcast_admin"
    ]
...
PERMISSION_ENDPOINTS = {
    ...
    "page_admin":"pages","page_new":"pages","page_edit":"pages","page_delete":"delete","reports_page":"reports","analytics_page":"analytics",
    "notifications_page":"notifications","audit_log":"audit","security_page":"security","insights_page":"insights","risk_page":"risk",
...
def has_permission(permission):
    ...
    if get_setting("permissionsEnabled", "1") != "1":
        # When granular permissions are disabled, administrators retain the
        # normal admin capabilities. The owner can enable the matrix when
        # finer-grained restrictions are required.
        return u["role"] in ("CREATOR", "SUPER_ADMIN", "ADMIN")
...
ADMIN_GET_ENDPOINTS = {
    ...
    "notifications_page", "analytics_page", "events_admin",
    "partners_admin", "media_admin", "security_page", "certificate", "certificate_view", "insights_page", "risk_page", "navigation_page", "navigation_delete", "sections_page", "permissions_page", "media_upload", "control_center", "upload_public_asset", "live_admin", "academy_payments_admin",
}
...
def admin_dashboard():
    ...
    stats={
        ...
        "hours": round(float(db.execute("SELECT COALESCE(SUM(hours),0) h FROM attendance").fetchone()["h"] or 0) + float(db.execute("SELECT COALESCE(SUM(hours),0) h FROM initiative_participants").fetchone()["h"] or 0), 2),
        "temporary_volunteers": db.execute("SELECT COUNT(*) c FROM temporary_volunteers WHERE status != 'inactive'").fetchone()["c"],
        "beneficiaries": db.execute("SELECT COUNT(DISTINCT member_id) c FROM initiative_participants WHERE member_id IS NOT NULL").fetchone()["c"],
    ...
    committees=db.execute("SELECT * FROM committees ORDER BY COALESCE(sort_order,0), name").fetchall()
...
def public_home():
    db = get_db()
    members = db.execute("SELECT * FROM members ORDER BY COALESCE(sort_order,0), name").fetchall()
    admins = db.execute("SELECT * FROM administrators ORDER BY COALESCE(sort_order,0), name").fetchall()
    ...
    attendance_hours = db.execute("SELECT COALESCE(SUM(hours),0) h FROM attendance WHERE status IN ('Present','Late')").fetchone()["h"] or 0
    participant_hours = db.execute("SELECT COALESCE(SUM(hours),0) h FROM initiative_participants").fetchone()["h"] or 0
    total_hours = round(float(attendance_hours) + float(participant_hours), 2)
    if not total_hours:
        total_hours = round(sum((i["hours"] or 0) for i in initiatives), 2)
    temp_count = db.execute("SELECT COUNT(*) c FROM temporary_volunteers WHERE status != 'inactive'").fetchone()["c"]
    beneficiaries_count = db.execute("SELECT COUNT(DISTINCT member_id) c FROM initiative_participants WHERE member_id IS NOT NULL").fetchone()["c"]
    news_count = db.execute("SELECT COUNT(*) c FROM news WHERE status='published'").fetchone()["c"]
    ...
    public_admins = db.execute("SELECT * FROM administrators ORDER BY COALESCE(sort_order,0), date ASC LIMIT 6").fetchall()
    ...
    return render_template("public_home.html",
        members_count=len(members), admins_count=len(admins), temporary_volunteers_count=temp_count, beneficiaries_count=int(get_setting("beneficiariesCount","0") or 0) or beneficiaries_count, news_count=news_count, initiatives_count=len(initiatives),
...
def impact_public():
    ...
    attendance_hours=db.execute("SELECT COALESCE(SUM(hours),0) h FROM attendance WHERE status IN ('Present','Late')").fetchone()["h"] or 0
    participant_hours=db.execute("SELECT COALESCE(SUM(hours),0) h FROM initiative_participants").fetchone()["h"] or 0
    hours=round(float(attendance_hours)+float(participant_hours),2)
    if not hours: hours=round(float(db.execute("SELECT COALESCE(SUM(hours),0) h FROM initiatives").fetchone()["h"] or 0),2)
    beneficiaries=db.execute("SELECT COUNT(DISTINCT member_id) c FROM initiative_participants WHERE member_id IS NOT NULL").fetchone()["c"]
    admins=db.execute("SELECT COUNT(*) c FROM administrators").fetchone()["c"]
    ...
    return render_template("impact.html", members=members, initiatives=initiatives, hours=round(hours), admins=admins, beneficiaries=beneficiaries, points=points)
...
def live_room(sid):
    ...
    if ls["mode"] == "external" and ls["external_url"]:
        return redirect(ls["external_url"])
    u=current_user()
    approved=False
    if u:
...
def temporary_volunteer_card(vid):
    return redirect(url_for("card_public",kind="temporary",record_id=vid))
...
def temporary_volunteers_admin():
    ...
    if request.method=="POST":
        ...
        vid=uid("tv")
        db.execute("""INSERT INTO temporary_volunteers(id,name,phone,department,stage,specialty,task,committee,initiative_id,start_date,end_date,hours,benefits,status,photo,card_public,qr_enabled,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(vid,request.form.get("name","").strip(),request.form.get("phone",""),request.form.get("department",""),request.form.get("stage",""),request.form.get("specialty",""),request.form.get("task",""),request.form.get("committee",""),request.form.get("initiative_id") or None,request.form.get("start_date") or None,request.form.get("end_date") or None,float(request.form.get("hours",0) or 0),request.form.get("benefits",""),"active",photo,1 if request.form.get("card_public") else 0,1 if request.form.get("qr_enabled") else 0,now_iso()))
    ...
    initiatives=db.execute("SELECT id,name FROM initiatives ORDER BY date DESC,name").fetchall()
    return render_template("temporary_volunteers_admin.html", volunteers=rows, initiatives=initiatives)
...
def events_admin():
    ...
    if request.method=="POST":
        ...
        responsible_key=request.form.get("responsible", "")
        rtype,rid,rname="member","",responsible_key
        if ":" in responsible_key:
            rtype,rid=responsible_key.split(":",1); table="administrators" if rtype=="admin" else "members"
            rr=db.execute(f"SELECT name FROM {table} WHERE id=?",(rid,)).fetchone(); rname=rr["name"] if rr else responsible_key
        db.execute("INSERT INTO events(id,title,date,time,location,description,status,cover_image,created_at,qr_path,latitude,longitude,map_visible,responsible,responsible_type,responsible_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(eid,request.form.get("title"),request.form.get("date"),request.form.get("time"),request.form.get("location"),request.form.get("description"),request.form.get("status","upcoming"),cover,now_iso(),_action_qr("event",eid,"action"),lat,lng,1 if request.form.get("map_visible") else 0,rname,rtype,rid))
    ...
    return render_template("events_admin.html",events=db.execute("SELECT * FROM events ORDER BY date DESC").fetchall(),members=db.execute("SELECT id,name FROM members ORDER BY name").fetchall(),admins=db.execute("SELECT id,name FROM administrators ORDER BY name").fetchall())

@app.route("/admin/events/<eid>/edit", methods=["GET","POST"])
def event_edit(eid):
    db=get_db(); e=db.execute("SELECT * FROM events WHERE id=?",(eid,)).fetchone()
    if not e: return redirect(url_for("events_admin"))
    if request.method=="POST":
        cover=e["cover_image"]
        new_cover=save_image_as_jpg(request.files.get("cover_file"), "event")
        ...
        if ":" in responsible_key:
            rtype,rid=responsible_key.split(":",1); table="administrators" if rtype=="admin" else "members"
            rr=db.execute(f"SELECT name FROM {table} WHERE id=?",(rid,)).fetchone(); rname=rr["name"] if rr else responsible_key
        db.execute("UPDATE events SET title=?,date=?,time=?,location=?,description=?,status=?,cover_image=?,latitude=?,longitude=?,map_visible=?,responsible=?,responsible_type=?,responsible_id=? WHERE id=?",(request.form.get("title"),request.form.get("date"),request.form.get("time"),request.form.get("location"),request.form.get("description"),request.form.get("status","upcoming"),cover,request.form.get("latitude") or None,request.form.get("longitude") or None,1 if request.form.get("map_visible") else 0,rname,rtype,rid,eid))
        save_multiple_attachments(request.files.getlist("media_files"), "event", eid, db, request.form.get("title"))
        db.commit(); log_action("Updated event",eid); return redirect(url_for("events_admin"))
    return render_template("events_admin.html",events=db.execute("SELECT * FROM events ORDER BY date DESC").fetchall(),edit_event=e,members=db.execute("SELECT id,name FROM members ORDER BY name").fetchall(),admins=db.execute("SELECT id,name FROM administrators ORDER BY name").fetchall())
...
def members_list():
    ...
    committees=db.execute("SELECT * FROM committees ORDER BY name").fetchall()
    temp_volunteers=db.execute("SELECT * FROM temporary_volunteers WHERE status='active' ORDER BY COALESCE(sort_order,0),name").fetchall()
    grouped=[]
    for c in committees:
        cname=(c["name"] or "").strip()
        grouped.append({"id":c["id"],"name":cname,"members":[m for m in members if (m["committee"] or "").strip()==cname],"volunteers":[v for v in temp_volunteers if (v["committee"] or "").strip()==cname]})
    assigned={m["id"] for g in grouped for m in g["members"]}
    assigned_v={v["id"] for g in grouped for v in g["volunteers"]}
    unassigned=[m for m in members if m["id"] not in assigned]
    unassigned_v=[v for v in temp_volunteers if v["id"] not in assigned_v]
    if unassigned or unassigned_v: grouped.append({"id":"","name":"بدون لجنة", "members":unassigned,"volunteers":unassigned_v})
    return render_template("members_list.html", data=data, q=q, grouped=grouped)
...
def member_form(mid=None):
    ...
    if request.method == "POST":
        ...
        data = (name, request.form.get("email", ""), request.form.get("phone", ""),
                ...
                request.form.get("card_expiry", "") or (member["card_expiry"] if member else ""), request.form.get("nfc_uid", "").strip(), 1 if request.form.get("card_public") else 0, 1 if request.form.get("qr_enabled") else 0)
        if mid:
            db.execute("UPDATE members SET name=?,email=?,phone=?,committee=?,position=?,join_date=?,status=?,notes=?,photo=?,department=?,stage=?,membership_no=?,card_issue_date=?,card_expiry=?,nfc_uid=?,card_public=?,qr_enabled=? WHERE id=?", data + (mid,))
            log_action("Updated", f"عضو: {name}")
        else:
            db.execute("INSERT INTO members(id,name,email,phone,committee,position,join_date,status,notes,photo,department,stage,membership_no,card_issue_date,card_expiry,nfc_uid,card_public,qr_enabled,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (uid("mem"),) + data + (now_iso(),))
...
def admin_form(aid=None):
    ...
    if request.method == "POST":
        ...
        extra_positions = [x.strip() for x in request.form.getlist("extra_position") if x.strip()]
        extra_committees = [x.strip() for x in request.form.getlist("extra_committee") if x.strip()]
        positions_payload = [{"position": p, "committee": (extra_committees[i] if i < len(extra_committees) else "")} for i,p in enumerate(extra_positions)]
        data = (name, request.form.get("position", ""), request.form.get("committee", ""),
                ...
                request.form.get("card_expiry", "") or (a["card_expiry"] if a else ""), request.form.get("nfc_uid", "").strip(), 1 if request.form.get("card_public") else 0, 1 if request.form.get("qr_enabled") else 0, json.dumps(positions_payload, ensure_ascii=False))
        if aid:
            db.execute("UPDATE administrators SET name=?,position=?,committee=?,date=?,responsibilities=?,photo=?,department=?,membership_no=?,card_issue_date=?,card_expiry=?,nfc_uid=?,card_public=?,qr_enabled=?,positions_json=? WHERE id=?", data + (aid,))
        else:
            db.execute("INSERT INTO administrators(id,name,position,committee,date,responsibilities,photo,department,membership_no,card_issue_date,card_expiry,nfc_uid,card_public,qr_enabled,positions_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (uid("adm"),) + data)
        db.commit()
        log_action("Updated" if aid else "Created", f"إداري: {name}")
        flash("تم الحفظ بنجاح", "ok")
        return redirect(url_for("admins_list"))
    extra_positions=[]
    if a and a["positions_json"]:
        try: extra_positions=json.loads(a["positions_json"] or "[]")
        except Exception: extra_positions=[]
    return render_template("admin_form.html", a=a, committees=committees, extra_positions=extra_positions)
...
def committees_list():
    ...
    data = []
    for c in committees:
        members=db.execute("SELECT id,name,photo,position FROM members WHERE committee=? ORDER BY COALESCE(sort_order,0),name", (c["name"],)).fetchall()
        volunteers=db.execute("SELECT id,name,photo,task,initiative_id FROM temporary_volunteers WHERE committee=? AND status='active' ORDER BY COALESCE(sort_order,0),name", (c["name"],)).fetchall()
        data.append({"c": c, "member_count": len(members), "volunteer_count": len(volunteers), "members": members, "volunteers": volunteers})
...
def committee_form(cid=None):
    ...
    if request.method == "POST":
        ...
        if cid:
            ...
            next_order = db.execute("SELECT COALESCE(MAX(sort_order),0)+10 AS n FROM committees").fetchone()["n"]
            db.execute("INSERT INTO committees(id,name,head,description,sort_order) VALUES(?,?,?,?,?)", (uid("com"),) + data + (next_order,))
        db.commit()
...
def initiative_view(iid):
    ...
    """, (iid,)).fetchall()
    temp_volunteers=db.execute("SELECT * FROM temporary_volunteers WHERE initiative_id=? AND status='active' ORDER BY name", (iid,)).fetchall()
    return render_template("initiative_view.html", i=i, participants=participants, temp_volunteers=temp_volunteers, public_view=not is_admin(), map_token=True)
...
def task_form(tid=None):
    ...
    if request.method == "POST":
        ...
        if not title:
            ...
            return redirect(request.url)
        assignee_key = request.form.get("assignee", "")
        assignee_type, assignee_id, assignee = "member", "", assignee_key
        if ":" in assignee_key:
            assignee_type, assignee_id = assignee_key.split(":",1)
            table = "administrators" if assignee_type == "admin" else "members"
            row = db.execute(f"SELECT name FROM {table} WHERE id=?", (assignee_id,)).fetchone()
            assignee = row["name"] if row else assignee_key
        status = request.form.get("status", "Todo")
        data = (title, assignee, assignee_type, assignee_id, request.form.get("deadline", ""),
        ...
        if tid:
            db.execute("""UPDATE tasks SET title=?,assignee=?,assignee_type=?,assignee_id=?,deadline=?,priority=?,status=?,description=?,latitude=?,longitude=?,map_visible=?
                           WHERE id=?""", data + (tid,))
        else:
            new_tid=uid("task")
            db.execute("""INSERT INTO tasks(id,title,assignee,assignee_type,assignee_id,deadline,priority,status,description,latitude,longitude,map_visible,created_at,qr_path)
    ...
    return render_template("task_form.html", t=t, members=members, admins=admins)
...
def attendance_list():
    ...
    for r in rows:
        person_name = None
        person_type = r["person_type"] if "person_type" in r.keys() and r["person_type"] else "member"
        person_id = r["person_id"] if "person_id" in r.keys() and r["person_id"] else r["member_id"]
        table = {"member":"members","admin":"administrators","temporary":"temporary_volunteers"}.get(person_type,"members")
        if person_id:
            m = db.execute(f"SELECT name FROM {table} WHERE id=?", (person_id,)).fetchone()
            person_name = m["name"] if m else None
        i = db.execute("SELECT name FROM initiatives WHERE id=?", (r["initiative_id"],)).fetchone() if r["initiative_id"] else None
        data.append({"a": r, "member_name": person_name or "—", "person_type": person_type, "ini_name": i["name"] if i else "—"})
...
def attendance_new():
    ...
    members = db.execute("SELECT * FROM members ORDER BY name").fetchall()
    admins = db.execute("SELECT * FROM administrators ORDER BY name").fetchall()
    temporary = db.execute("SELECT * FROM temporary_volunteers WHERE status != 'inactive' ORDER BY name").fetchall()
    if not members and not admins and not temporary:
        flash("أضف شخصًا واحدًا على الأقل أولاً", "error")
        return redirect(url_for("attendance_list"))
    initiatives = db.execute("SELECT * FROM initiatives ORDER BY date DESC").fetchall()
    if request.method == "POST":
        person_key = request.form.get("person_key", "")
        person_type, person_id = (person_key.split(":",1)+[""])[:2] if ":" in person_key else ("member", request.form.get("member_id"))
        if person_type not in ("member","admin","temporary") or not person_id:
            flash("اختر الشخص أولاً", "error"); return redirect(url_for("attendance_new"))
        table={"member":"members","admin":"administrators","temporary":"temporary_volunteers"}[person_type]
        person=db.execute(f"SELECT id,name FROM {table} WHERE id=?",(person_id,)).fetchone()
        if not person:
            flash("الشخص غير موجود", "error"); return redirect(url_for("attendance_new"))
        status = request.form.get("status", "Present")
        att_date = iraq_today()
        initiative_id = request.form.get("initiative_id") or None
        start_time = iraq_now_time() if status in ("Present", "Late") else None
        end_time = None; hours = 0.0
        db.execute("INSERT INTO attendance(id,member_id,date,status,initiative_id,start_time,end_time,hours,person_type,person_id,event_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                   (uid("att"), person_id if person_type=="member" else None, att_date, status, initiative_id, start_time, end_time, hours, person_type, person_id, None))
        db.commit()
        if status == "Present" and person_type == "member":
            pts_cfg = get_points_config(); add_points(person_id, pts_cfg.get("attendance", 10), f"حضور بتاريخ {att_date}")
        log_action("Created", "حضور مسجل")
        flash("تم تسجيل الدخول تلقائيًا بالوقت الحالي", "ok")
        return redirect(url_for("attendance_list"))
    return render_template("attendance_form.html", members=members, admins=admins, temporary=temporary, initiatives=initiatives, today=iraq_today())
...
# ============================================================ MEMBER / ADMIN CARDS ============================================================
def _identity_qr(kind, record_id, number=""):
    """Generate the permanent QR identity printed inside an electronic AHVT ID.
    The QR resolves to the person's AHVT scan gateway, so the same code works
    for attendance, tasks, events and equipment after the server resolves the
    person type/id. It is regenerated safely from the same stable filename.
    """
    target=url_for("scan_record", kind=kind, record_id=record_id, _external=True)
    filename=f"identity-{kind}-{secure_filename(str(record_id))}.png"
...
def _card_qr(kind, record_id, number):
    return _identity_qr(kind, record_id, number)

def _temporary_volunteer_qr(record_id):
    return _identity_qr("temporary", record_id)
...
def cards_admin():
    ...
    temporary=db.execute("SELECT * FROM temporary_volunteers ORDER BY name").fetchall()
    return render_template("cards_admin.html",members=members,admins=admins,temporary=temporary,templates=templates)
...
def card_template_preview(tid):
    ...
    kind=t["kind"]
    if kind=="temporary":
        person=db.execute("SELECT * FROM temporary_volunteers ORDER BY name LIMIT 1").fetchone()
        if not person:return "أضف متطوعًا وقتيًا أولاً لمعاينة البطاقة",404
        qr=_temporary_volunteer_qr(person["id"]); nfc_url=url_for("card_public",kind="temporary",record_id=person["id"],_external=True)
        return render_template("membership_card.html",person=person,kind="temporary",qr_path=qr,nfc_url=nfc_url,team_name=get_setting("teamName","فريق الحكمة التطوعي"),template=t,printable=True)
...
@app.route("/admin/cards/temporary/<record_id>")
def temporary_card_print(record_id):
    if not has_permission("cards"): return render_template("403.html",permission="cards"),403
    db=get_db(); row=db.execute("SELECT * FROM temporary_volunteers WHERE id=?",(record_id,)).fetchone()
    if not row: return "السجل غير موجود",404
    qr=_temporary_volunteer_qr(record_id); tpl=_card_template(db,"temporary"); nfc_url=url_for("card_public",kind="temporary",record_id=record_id,_external=True)
    return render_template("membership_card.html", person=row, kind="temporary", qr_path=qr, nfc_url=nfc_url, team_name=get_setting("teamName","فريق الحكمة التطوعي"), template=tpl, printable=True)

@app.route("/card/temporary/<record_id>")
def temporary_card_public(record_id):
    return redirect(url_for("card_public",kind="temporary",record_id=record_id))

@app.route("/admin/cards/<kind>/<record_id>")
def card_print(kind, record_id):
    if not has_permission("cards"): return render_template("403.html",permission="cards"),403
    db=get_db(); table={"member":"members","admin":"administrators","temporary":"temporary_volunteers"}.get(kind,"" )
    if not table:return "نوع البطاقة غير صحيح",404
    row=db.execute(f"SELECT * FROM {table} WHERE id=?",(record_id,)).fetchone()
    if not row:return "السجل غير موجود",404
    if kind=="temporary": number=f"HIKMA-T-{record_id[-8:].upper()}"; qr=_temporary_volunteer_qr(record_id)
    else: number=row["membership_no"] or (f"HIKMA-M-{record_id[-8:].upper()}" if kind=="member" else f"HIKMA-A-{record_id[-8:].upper()}"); qr=_card_qr(kind,record_id,number)
    nfc_url=url_for("card_public",kind=kind,record_id=record_id,_external=True); tpl=_card_template(db,kind)
...
def card_public(kind, record_id):
    ...
    if kind=="temporary":
        number=f"HIKMA-T-{record_id[-8:].upper()}"; qr=_temporary_volunteer_qr(record_id)
    else:
        number=row["membership_no"] or (f"HIKMA-M-{record_id[-8:].upper()}" if kind=="member" else f"HIKMA-A-{record_id[-8:].upper()}"); qr=_card_qr(kind,record_id,number)
    nfc_url=url_for("card_public",kind=kind,record_id=record_id,_external=True); tpl=_card_template(db,kind)
    return render_template("membership_card.html", person=row, kind=kind, qr_path=qr, nfc_url=nfc_url, team_name=get_setting("teamName","فريق الحكمة التطوعي"), template=tpl, printable=False, qr_public_enabled=get_setting("qrPublicEnabled","1") == "1")
...
def resolve_qr_person(db, kind, rid):
    """Resolve a person encoded by an AHVT QR card: member, administrator, or temporary volunteer."""
    table = {"member": "members", "admin": "administrators", "temporary": "temporary_volunteers"}.get(kind)
    if not table:
        return None
    row = db.execute(f"SELECT * FROM {table} WHERE id=?", (rid,)).fetchone()
    if not row:
        return None
    return {"row": row, "type": kind, "id": rid, "name": row["name"]}
...
def admin_qr_scan_api():
    ...
    if kind not in ("member","admin","temporary","equipment","task","event"):
        return jsonify({"ok":False,"error":"هذا QR ليس من بطاقات AHVT المدعومة"}),400
    person = resolve_qr_person(db, kind, rid) if kind in ("member","admin","temporary") else None
    if kind in ("member","admin","temporary") and not person:
        return jsonify({"ok":False,"error":"الشخص غير موجود في سجلات AHVT"}),404
    if kind in ("member","admin","temporary") and mode in ("attendance","event"):
        today=iraq_today()
        open_att=db.execute("SELECT * FROM attendance WHERE person_type=? AND person_id=? AND date=? AND end_time IS NULL ORDER BY start_time DESC LIMIT 1",(kind,rid,today)).fetchone()
        if open_att:
            start_time=open_att["start_time"] or iraq_now_time(); end=iraq_now_time()
            try: h=max(0,(datetime.strptime(end,"%H:%M")-datetime.strptime(start_time,"%H:%M")).seconds/3600)
            ...
            db.execute("INSERT INTO attendance(id,member_id,date,status,initiative_id,start_time,end_time,hours,person_type,person_id,event_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(uid("att"),rid if kind=="member" else None,today,"Present",None,iraq_now_time(),None,0.0,kind,rid,record_id if mode=="event" else None)); action="تسجيل الدخول"
        db.commit(); log_action("Camera QR attendance",f"{person['name']} · {action}")
        return jsonify({"ok":True,"message":f"{action}: {person['name']}","name":person["name"],"person_type":kind,"department":person["row"]["department"] if "department" in person["row"].keys() else "","committee":person["row"]["committee"] if "committee" in person["row"].keys() else "","time":iraq_now_time()})
    if mode=="equipment" and person:
        eid=equipment_id or record_id
        if not eid:
            return jsonify({"ok":False,"error":"اختر المعدة قبل المسح"}),400
        e=db.execute("SELECT * FROM podcast_equipment WHERE id=?",(eid,)).fetchone()
        if not e:return jsonify({"ok":False,"error":"المعدة غير موجودة"}),404
        open_loan=db.execute("SELECT id FROM equipment_loans WHERE equipment_id=? AND person_type=? AND person_id=? AND status='issued' ORDER BY issued_at DESC LIMIT 1",(eid,kind,rid)).fetchone()
        if open_loan:
            db.execute("UPDATE equipment_loans SET status='returned',returned_at=? WHERE id=?",(now_iso(),open_loan["id"])); action="إرجاع"
        else:
            db.execute("INSERT INTO equipment_loans(id,equipment_id,member_id,issued_at,status,notes,person_type,person_id) VALUES(?,?,?,?,?,?,?,?)",(uid("loan"),eid,rid if kind=="member" else None,now_iso(),"issued","تم التسجيل عبر كاميرا QR",kind,rid)); action="استلام"
        db.commit(); log_action(f"Equipment {action} via camera QR",f"{person['name']} · {e['name']}")
        return jsonify({"ok":True,"message":f"تم {action} {e['name']} — {person['name']}","name":person["name"],"time":iraq_now_time()})
    if mode=="task" and person:
        tid=record_id; t=db.execute("SELECT * FROM tasks WHERE id=?",(tid,)).fetchone()
        if not t:return jsonify({"ok":False,"error":"المهمة غير موجودة"}),404
        exists=db.execute("SELECT id FROM task_completions WHERE task_id=? AND person_type=? AND person_id=?",(tid,kind,rid)).fetchone()
        if not exists:
            db.execute("INSERT INTO task_completions(id,task_id,member_id,completed_at,notes,person_type,person_id) VALUES(?,?,?,?,?,?,?)",(uid("tc"),tid,rid if kind=="member" else None,now_iso(),"تم عبر كاميرا QR",kind,rid)); db.commit()
        log_action("Task camera QR",f"{person['name']} · {t['title']}")
        return jsonify({"ok":True,"message":f"تم تسجيل المهمة: {t['title']} — {person['name']}","name":person["name"],"time":iraq_now_time()})
    if mode in ("attendance","event","task","equipment") and kind not in ("member","admin","temporary"):
        return jsonify({"ok":False,"error":"امسح QR بطاقة شخص من AHVT"}),400
    return jsonify({"ok":False,"error":"السياق غير مدعوم"}),400

def _person_certificates(db, kind, record_id):
    if kind == "member":
        return db.execute("SELECT * FROM certificates WHERE recipient_type='member' AND recipient_person_id=? OR (recipient_member_id=? AND (recipient_type IS NULL OR recipient_type='member')) ORDER BY created_at DESC", (record_id, record_id)).fetchall()
    return db.execute("SELECT * FROM certificates WHERE recipient_type=? AND recipient_person_id=? ORDER BY created_at DESC", (kind, record_id)).fetchall()

def _person_identity_counts(db, kind, record_id):
    attendance_count=db.execute("SELECT COUNT(*) c FROM attendance WHERE person_type=? AND person_id=?",(kind,record_id)).fetchone()["c"]
    hours=db.execute("SELECT COALESCE(SUM(hours),0) h FROM attendance WHERE person_type=? AND person_id=?",(kind,record_id)).fetchone()["h"] or 0
    initiative_count=db.execute("SELECT COUNT(*) c FROM initiative_participants WHERE member_id=?",(record_id,)).fetchone()["c"] if kind=="member" else 0
    return attendance_count, initiative_count, round(float(hours),2)

@app.route("/scan/<kind>/<record_id>")
def scan_record(kind, record_id):
    db=get_db()
    if kind in ("member","admin","temporary"):
        table = {"member":"members","admin":"administrators","temporary":"temporary_volunteers"}[kind]
        row=db.execute(f"SELECT * FROM {table} WHERE id=?",(record_id,)).fetchone()
        if not row: return "السجل غير موجود",404
        if not row["card_public"] and not is_admin():
            return render_template("403.html",permission="البطاقة خاصة"),403
        # Permanent card QR: the QR itself never changes; this page always reads
        # the latest record, so updated member/admin/volunteer information appears
        # immediately without reprinting the physical card.
        attendance_count=db.execute("SELECT COUNT(*) c FROM attendance WHERE person_type=? AND person_id=?",(kind,record_id)).fetchone()["c"]
        initiative_count=db.execute("SELECT COUNT(*) c FROM initiative_participants WHERE member_id=?",(record_id,)).fetchone()["c"] if kind=="member" else 0
        hours=db.execute("SELECT COALESCE(SUM(hours),0) h FROM attendance WHERE person_type=? AND person_id=?",(kind,record_id)).fetchone()["h"] or 0
        certs=_person_certificates(db,kind,record_id)
        return render_template("identity.html", person=row, kind=kind, attendance_count=attendance_count, initiative_count=initiative_count, volunteer_hours=round(float(hours),2), certificates=certs, team_name=get_setting("teamName","فريق الحكمة التطوعي"), extra_positions=(json.loads(row["positions_json"] or "[]") if kind=="admin" and row["positions_json"] else []))
    if kind=="temporary":
        row=db.execute("SELECT * FROM temporary_volunteers WHERE id=?",(record_id,)).fetchone()
        if not row:return "المتطوع غير موجود",404
        if not row["card_public"] and not is_admin(): return render_template("403.html",permission="البطاقة خاصة"),403
        return render_template("temporary_volunteer_card.html", volunteer=row, qr_path=_temporary_volunteer_qr(record_id), printable=False)
    if kind=="event":
...
@app.post("/scan/person/<kind>/<record_id>/action")
def scan_person_action(kind, record_id):
    if not is_admin(): return render_template("403.html",permission="scan"),403
    db=get_db(); person=resolve_qr_person(db,kind,record_id)
    if not person:return "الشخص غير موجود",404
    action=request.form.get("action","attendance"); today=iraq_today(); now=iraq_now_time()
    if action=="attendance":
        existing=db.execute("SELECT id FROM attendance WHERE person_type=? AND person_id=? AND date=? AND end_time IS NULL ORDER BY start_time DESC LIMIT 1",(kind,record_id,today)).fetchone()
        if existing:
            db.execute("UPDATE attendance SET end_time=?,hours=? WHERE id=?",(now,0,existing["id"])); msg="تم تسجيل الخروج"
        else:
            db.execute("INSERT INTO attendance(id,member_id,date,status,initiative_id,start_time,end_time,hours,person_type,person_id,event_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(uid("att"),record_id if kind=="member" else None,today,"Present",request.form.get("initiative_id") or None,now,None,0.0,kind,record_id,None)); msg="تم تسجيل الحضور"
    elif action=="task":
        tid=request.form.get("task_id"); t=db.execute("SELECT * FROM tasks WHERE id=?",(tid,)).fetchone()
        if not t:return "المهمة غير موجودة",404
        db.execute("INSERT OR IGNORE INTO task_completions(id,task_id,member_id,completed_at,notes,person_type,person_id) VALUES(?,?,?,?,?,?,?)",(uid("tc"),tid,record_id if kind=="member" else None,now_iso(),request.form.get("notes",""),kind,record_id)); msg="تم تسجيل إنجاز المهمة"
        ...
        open_loan=db.execute("SELECT id FROM equipment_loans WHERE equipment_id=? AND person_type=? AND person_id=? AND status='issued' ORDER BY issued_at DESC LIMIT 1",(eid,kind,record_id)).fetchone()
        if open_loan:
            db.execute("UPDATE equipment_loans SET status='returned',returned_at=? WHERE id=?",(now_iso(),open_loan["id"])); msg="تم تسجيل الإرجاع"
        else:
            db.execute("INSERT INTO equipment_loans(id,equipment_id,member_id,issued_at,status,notes,person_type,person_id) VALUES(?,?,?,?,?,?,?,?)",(uid("loan"),eid,record_id if kind=="member" else None,now_iso(),"issued","تم الاستلام عبر QR",kind,record_id)); msg="تم تسجيل الاستلام"
    else: return "العملية غير مدعومة",400
    db.commit(); log_action("QR person action",f"{person['name']} · {action}"); flash(f"{msg}: {person['name']}","ok")
...
@app.route("/admin/honor/<hid>/gallery/<attachment_id>/delete", methods=["POST"])
def honor_gallery_delete(hid, attachment_id):
    if not has_permission("delete"): return render_template("403.html", permission="delete"),403
    db=get_db(); row=db.execute("SELECT * FROM attachments WHERE id=? AND owner_type=? AND owner_id=?",(attachment_id,"honor",hid)).fetchone()
    if not row: flash("الصورة غير موجودة","error"); return redirect(url_for("honor_edit",hid=hid))
    path=row["file_path"] or row["url"] or ""
    if path.startswith("uploads/"):
        try: os.remove(os.path.join(UPLOAD_DIR,path.split("uploads/",1)[1]))
        except OSError: pass
    db.execute("DELETE FROM attachments WHERE id=?",(attachment_id,)); db.commit(); log_action("Deleted honor gallery image",attachment_id)
    flash("تم حذف الصورة من معرض التكريم","ok"); return redirect(url_for("honor_edit",hid=hid))
...
def certificates_admin():
    ...
    if request.method=="POST":
        ...
        cid=uid("cert"); token=uuid.uuid4().hex; u=current_user()
        ctype=request.form.get("certificate_type","شهادة تكريم")
        recipient_type=request.form.get("recipient_type","member")
        recipient_person_id=request.form.get("recipient_person_id") or None
        member_id=recipient_person_id if recipient_type=="member" else None
        if recipient_type=="external":
            ext_id=recipient_person_id
            if not ext_id:
                ext_id=uid("ext")
                db.execute("INSERT INTO external_people(id,name,organization,role,created_at,status) VALUES(?,?,?,?,?,?)",(ext_id,recipient,request.form.get("organization","").strip(),request.form.get("external_role","").strip(),now_iso(),"active"))
                recipient_person_id=ext_id
            else:
                db.execute("UPDATE external_people SET name=?,organization=?,role=? WHERE id=?",(recipient,request.form.get("organization","").strip(),request.form.get("external_role","").strip(),ext_id))
        db.execute("""INSERT INTO certificates(id,certificate_no,recipient_name,certificate_type,initiative_id,initiative_name,issue_date,hours,note,issued_by,created_at,status,verify_token,recipient_member_id,recipient_type,recipient_person_id)
                      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                   (cid,no,recipient,ctype,initiative_id,ini["name"] if ini else request.form.get("initiative_name","").strip(),issue_date,hours,request.form.get("note","").strip(),u["name"] if u else get_setting("teamName","فريق الحكمة التطوعي"),now_iso(),"valid",token,member_id,recipient_type,recipient_person_id))
        db.commit()
        qr_path=_certificate_qr(cid,no)
        db.execute("UPDATE certificates SET qr_path=? WHERE id=?",(qr_path,cid)); db.commit()
        log_action("Created certificate barcode",f"{recipient} · {no}")
        flash("تم إنشاء باركود الشهادة","ok")
    ...
    return render_template("certificates_admin.html",certificates=rows,initiatives=db.execute("SELECT id,name FROM initiatives ORDER BY date DESC").fetchall(),members=db.execute("SELECT id,name FROM members ORDER BY name").fetchall(),admins=db.execute("SELECT id,name FROM administrators ORDER BY name").fetchall(),temporary=db.execute("SELECT id,name FROM temporary_volunteers ORDER BY name").fetchall(),external_people=db.execute("SELECT id,name,organization FROM external_people ORDER BY name").fetchall(),today=date.today().isoformat(),edit_certificate=None)
...
def certificate_edit(cid):
    ...
    if request.method=="POST":
        recipient=request.form.get("recipient_name","").strip()
        rtype=request.form.get("recipient_type", c["recipient_type"] or "member")
        rid=request.form.get("recipient_person_id") or None
        if rtype=="external" and rid:
            db.execute("UPDATE external_people SET name=?,organization=?,role=? WHERE id=?",(recipient,request.form.get("organization","").strip(),request.form.get("external_role","").strip(),rid))
        db.execute("UPDATE certificates SET recipient_name=?,certificate_type=?,initiative_name=?,issue_date=?,hours=?,note=?,recipient_member_id=?,recipient_type=?,recipient_person_id=? WHERE id=?",(recipient,request.form.get("certificate_type","شهادة تكريم"),request.form.get("initiative_name","").strip(),request.form.get("issue_date") or date.today().isoformat(),float(request.form.get("hours") or 0),request.form.get("note","").strip(),rid if rtype=="member" else None,rtype,rid,cid))
        db.commit(); log_action("Edited certificate barcode",c["certificate_no"]); flash("تم تحديث بيانات الباركود","ok"); return redirect(url_for("certificate_view",cid=cid))
    return render_template("certificates_admin.html",certificates=[],initiatives=db.execute("SELECT id,name FROM initiatives ORDER BY date DESC").fetchall(),members=db.execute("SELECT id,name FROM members ORDER BY name").fetchall(),admins=db.execute("SELECT id,name FROM administrators ORDER BY name").fetchall(),temporary=db.execute("SELECT id,name FROM temporary_volunteers ORDER BY name").fetchall(),external_people=db.execute("SELECT id,name,organization FROM external_people ORDER BY name").fetchall(),today=date.today().isoformat(),edit_certificate=c)
...
def certificate_reissue(cid):
    ...
    db=get_db(); c=db.execute("SELECT * FROM certificates WHERE id=?",(cid,)).fetchone()
    if not c:return "الشهادة غير موجودة",404
    db.execute("UPDATE certificates SET status='valid' WHERE id=?",(cid,)); db.commit(); log_action("Reactivated certificate barcode",c["certificate_no"]); flash("تم تفعيل باركود الشهادة","ok"); return redirect(url_for("certificates_admin"))
...
def certificate_view(cid):
    ...
    if not c:return "الشهادة غير موجودة",404
    if not c["qr_path"]:
        qr=_certificate_qr(cid,c["certificate_no"]); db.execute("UPDATE certificates SET qr_path=? WHERE id=?",(qr,cid)); db.commit(); c=db.execute("SELECT * FROM certificates WHERE id=?",(cid,)).fetchone()
    return render_template("certificate_barcode.html",certificate=c)

@app.route("/verify/certificate/<certificate_no>")
def certificate_verify(certificate_no):
    # Verification is public, but only an existing certificate number is
    # meaningful. Never expose internal IDs, tokens, or editing controls here.
    c=get_db().execute("SELECT certificate_no,recipient_name,certificate_type,initiative_name,issue_date,hours,status,issuer_name,writer_name,qr_path FROM certificates WHERE certificate_no=?",(certificate_no,)).fetchone()
...
def live_recording_upload(sid):
    if get_setting("liveRecordingEnabled", "1") != "1" or not has_permission("live_recordings"):
        return jsonify({"ok":False,"error":"Live recording permission required"}),403
...
def live_admin():
    ...
    db=get_db()
    if request.method=="POST":
        action=request.form.get("action","start_external")
        if action=="stop":
            ...
            if sid:
                ...
                db.commit(); log_action("Stopped external live broadcast",sid)
        ...
        title=request.form.get("live_title","بث مباشر").strip() or "بث مباشر"
        desc=request.form.get("live_description","").strip()
        ext=request.form.get("live_url","").strip()
        platform=request.form.get("live_platform","YouTube").strip()
        links=[x.strip() for x in request.form.getlist("external_links") if x.strip()]
        if ext: links.insert(0,ext)
        sid=uid("live")
        db.execute("""INSERT INTO live_sessions(id,title,description,mode,external_url,external_platform,status,created_by,started_at,created_at,external_links,viewer_peak)
                      VALUES(?,?,?,?,?,?,?,?,?,?,?,0)""",(sid,title,desc,"external",ext,platform,"live",current_user()["id"],now_iso(),now_iso(),json.dumps(links,ensure_ascii=False)))
        db.commit(); set_setting("liveEnabled","1"); set_setting("liveMode","external"); set_setting("liveSessionId",sid); set_setting("liveTitle",title); set_setting("liveDescription",desc); set_setting("liveUrl",ext); set_setting("livePlatform",platform)
        log_action("Added external live broadcast",title); flash("تمت إضافة البث","ok"); return redirect(url_for("live_admin"))
    active_sessions=db.execute("SELECT * FROM live_sessions WHERE status='live' ORDER BY started_at DESC LIMIT 6").fetchall()
    sessions=db.execute("SELECT * FROM live_sessions ORDER BY created_at DESC LIMIT 50").fetchall()
    return render_template("live_admin.html",active_sessions=active_sessions,sessions=sessions,source_requests=[],initiatives=[])
...
def podcast_admin():
    ...
    if request.method=="POST":
        ...
        if action=="show":
            ...
            eid=request.form.get("episode_id")
            e=db.execute("SELECT * FROM podcast_episodes WHERE id=?",(eid,)).fetchone()
            if not e: flash("الحلقة غير موجودة", "error"); return redirect(url_for("podcast_admin"))
            cover=_podcast_cover(request.files.get("cover")) or e["cover_image"]
            audio=save_uploaded_audio(request.files.get("audio"),"podcast-audio") or e["audio_file"]
            video=save_uploaded_video(request.files.get("video"),"podcast-video") or e["video_file"]
            status="published" if request.form.get("publish") else "draft"
            pub=e["published_at"] or (now_iso() if status=="published" else None)
            db.execute("UPDATE podcast_episodes SET show_id=?,season=?,episode_no=?,title=?,description=?,guest_id=?,host=?,director=?,producer=?,recorded_at=?,published_at=?,duration=?,cover_image=?,audio_file=?,video_file=?,transcript=?,status=?,updated_at=? WHERE id=?",(request.form.get("show_id") or None,int(request.form.get("season") or 1),int(request.form.get("episode_no") or 1),request.form.get("title"),request.form.get("description"),request.form.get("guest_id") or None,request.form.get("host"),request.form.get("director"),request.form.get("producer"),request.form.get("recorded_at"),pub,request.form.get("duration"),cover,audio,video,request.form.get("transcript"),status,now_iso(),eid))
        elif action=="equipment":
    ...
    edit_episode=db.execute("SELECT * FROM podcast_episodes WHERE id=?",(edit_id,)).fetchone() if edit_id else None
    return render_template("podcast_admin.html",shows=db.execute("SELECT * FROM podcast_shows ORDER BY updated_at DESC").fetchall(),guests=db.execute("SELECT * FROM podcast_guests ORDER BY name").fetchall(),episodes=db.execute("SELECT e.*,g.name guest_name,s.title show_title FROM podcast_episodes e LEFT JOIN podcast_guests g ON g.id=e.guest_id LEFT JOIN podcast_shows s ON s.id=e.show_id ORDER BY e.created_at DESC").fetchall(),equipment=db.execute("SELECT * FROM podcast_equipment ORDER BY category,name").fetchall(),edit_episode=edit_episode)
...
def card_visibility(kind,record_id):
    if not has_permission("cards"): return render_template("403.html",permission="cards"),403
    table="members" if kind=="member" else "administrators" if kind=="admin" else "temporary_volunteers" if kind=="temporary" else ""
    if not table:return "نوع البطاقة غير صحيح",404
...
@app.route("/admin/cards/<kind>/<record_id>/qr", methods=["POST"])
def card_qr_visibility(kind,record_id):
    if not has_permission("cards"): return render_template("403.html",permission="cards"),403
    table="members" if kind=="member" else "administrators" if kind=="admin" else "temporary_volunteers" if kind=="temporary" else ""
    if not table:return "نوع البطاقة غير صحيح",404
    db=get_db(); db.execute(f"UPDATE {table} SET qr_enabled=? WHERE id=?",(1 if request.form.get("qr_enabled") else 0,record_id)); db.commit(); flash("تم تحديث تشغيل QR","ok"); return redirect(url_for("cards_admin"))
...
def settings_page():
    ...
    if request.method == "POST":
        ...
        if form_type == "identity":
            ...
            # Do not overwrite uploaded background/video assets when the appearance form is saved.
            for key, field in [("accentColor","accent_color"),("navyColor","navy_color"),("backgroundColor","background_color"),("fontFamily","font_family"),("heroTitle","hero_title"),("heroText","hero_text"),("announcement","announcement"),("customCss","custom_css"),("seoTitle","seo_title"),("seoDescription","seo_description"),("certificateIssuer","certificate_issuer")]:
            ...
            for key, field in [("joinButtonVisible","join_visible"),("joinButtonText","join_text"),("joinButtonIcon","join_icon"),("joinButtonMode","join_mode"),("adminLinkVisible","admin_link_visible"),("telegramUrl","telegram_url"),("showPublicAdmins","show_admins"),("showPublicNews","show_news")]:
                default = "0" if field in ("join_visible","admin_link_visible","show_admins","show_news") else get_setting(key, "")
                set_setting(key, request.form.get(field, default))
            flash("تم تحديث زر الانضمام والتحكم العام", "ok")
        elif form_type == "about":
            for key, field in [("siteDescription","site_description"),("teamVision","team_vision"),("teamMission","team_mission"),("teamSlogan","team_slogan")]:
                set_setting(key, request.form.get(field, ""))
            flash("تم حفظ معلومات الفريق", "ok")
        elif form_type == "impact_controls":
            value=request.form.get("beneficiaries_count", "0").strip()
            try: value=str(max(0,int(value or 0)))
            except Exception: value="0"
            set_setting("beneficiariesCount", value)
            set_setting("qrPublicEnabled", "1" if request.form.get("qr_public_enabled") else "0")
            set_setting("liveRecordingEnabled", "1" if request.form.get("live_recording_enabled") else "0")
            flash("تم حفظ الإحصائيات والتحكم بالـ QR وحفظ البثوث", "ok")
...
def _report_rows(db, start=None, end=None):
    ...
    temporary_volunteers = filtered("SELECT * FROM temporary_volunteers ORDER BY created_at DESC") if _table_exists(db, "temporary_volunteers") else []
    equipment = filtered("SELECT * FROM podcast_equipment ORDER BY category, name") if _table_exists(db, "podcast_equipment") else []
    equipment_loans = filtered("""SELECT l.*, e.name equipment_name, COALESCE(m.name,a.name,v.name,'—') member_name
        FROM equipment_loans l LEFT JOIN podcast_equipment e ON e.id=l.equipment_id
        LEFT JOIN members m ON m.id=l.member_id
        LEFT JOIN administrators a ON a.id=l.person_id AND l.person_type='admin'
        LEFT JOIN temporary_volunteers v ON v.id=l.person_id AND l.person_type='temporary'
        ORDER BY l.issued_at DESC""") if _table_exists(db, "equipment_loans") else []
    ...
    if start or end:
        ...
        temporary_volunteers = [r for r in temporary_volunteers if in_period(r, "start_date") or in_period(r, "end_date") or in_period(r, "created_at")]
        equipment_loans = [r for r in equipment_loans if in_period(r, "issued_at") or in_period(r, "returned_at")]
...
def _report_pdf(path, data, start=None, end=None):
    ...
    stats = [
        ...
        ("المتطوعون الوقتيون", len(data["temporary_volunteers"])), ("المعدات", len(data["equipment"])),
        ("سجلات المعدات", len(data["equipment_loans"])), ("حلقات البودكاست", len(data["podcast"])),
        ("جلسات البث", len(data["live"]))
    ]
    stat_cells=[]
...
def _report_xlsx(path, data):
    ...
    sheet("المتطوعون الوقتيون", ["الاسم","القسم","المرحلة","التخصص","المهمة","اللجنة","من","إلى","الساعات","الحالة"], [[r["name"],r["department"],r["stage"],r["specialty"],r["task"],r["committee"],r["start_date"],r["end_date"],r["hours"],r["status"]] for r in data["temporary_volunteers"]])
    sheet("المعدات", ["المعدة","الفئة","الكمية","الحالة","ملاحظات"], [[r["name"],r["category"],r["quantity"],r["status"],r["notes"]] for r in data["equipment"]])
    sheet("سجل المعدات", ["المعدة","العضو","الحالة","وقت الاستلام","وقت الإرجاع","ملاحظات"], [[r["equipment_name"],r["member_name"],r["status"],r["issued_at"],r["returned_at"],r["notes"]] for r in data["equipment_loans"]])
    sheet("البودكاست", ["العنوان","المضيف","الضيف","النشر","الحالة","المدة"], [[r["title"],r["host"],r["guest_id"],r["published_at"],r["status"],r["duration"]] for r in data["podcast"]])
    sheet("البث المباشر", ["العنوان","الوضع","الحالة","البداية","النهاية","الرابط"], [[r["title"],r["mode"],r["status"],r["started_at"],r["ended_at"],r["external_url"]] for r in data["live"]])
    wb.save(path)
        ...
        if action=="course":
            ...
            for f in request.files.getlist("resource_files"):
                ...
                if ext in {".jpg",".jpeg",".png",".webp",".gif"}:
                    ...
                    namef=f"academy-resource-{uuid.uuid4().hex[:14]}{ext}"; f.save(os.path.join(UPLOAD_DIR,namef)); path=f"uploads/{namef}"; rtype="file"
                else: continue
                if path:
...
def equipment_admin():
    ...
    if request.method=="POST":
        ...
        if action=="add":
            ...
            eid=request.form.get("equipment_id"); row=db.execute("SELECT * FROM podcast_equipment WHERE id=?",(eid,)).fetchone()
            if not row: flash("المعدة غير موجودة","error")
            else:
                db.execute("UPDATE podcast_equipment SET category=?,name=?,quantity=?,status=?,notes=? WHERE id=?",(request.form.get("category"),request.form.get("name"),int(request.form.get("quantity") or 1),request.form.get("status","available"),request.form.get("notes"),eid)); db.commit(); log_action("Edited equipment",row["name"]); flash("تم تعديل المعدة","ok")
        elif action=="delete":
            eid=request.form.get("equipment_id"); row=db.execute("SELECT * FROM podcast_equipment WHERE id=?",(eid,)).fetchone()
            active=db.execute("SELECT 1 FROM equipment_loans WHERE equipment_id=? AND status='issued' LIMIT 1",(eid,)).fetchone()
            if active: flash("لا يمكن حذف معدة عليها إعارة نشطة","error")
            elif row:
                path=row["qr_path"] or ""
                if path.startswith("uploads/"):
                    try: os.remove(os.path.join(UPLOAD_DIR,path.split("uploads/",1)[1]))
                    except OSError: pass
                db.execute("DELETE FROM podcast_equipment WHERE id=?",(eid,)); db.commit(); log_action("Deleted equipment",row["name"]); flash("تم حذف المعدة","ok")
        elif action=="loan":
    ...
    db.commit()
    equipment_rows=db.execute("SELECT * FROM podcast_equipment ORDER BY category,name").fetchall()
    return render_template("equipment_admin.html",equipment=equipment_rows,loans=db.execute("""SELECT l.*,e.name equipment_name,COALESCE(m.name,a.name,v.name,'—') member_name FROM equipment_loans l LEFT JOIN podcast_equipment e ON e.id=l.equipment_id LEFT JOIN members m ON m.id=l.member_id LEFT JOIN administrators a ON a.id=l.person_id AND l.person_type='admin' LEFT JOIN temporary_volunteers v ON v.id=l.person_id AND l.person_type='temporary' ORDER BY l.issued_at DESC LIMIT 100""").fetchall(),members=db.execute("SELECT id,name FROM members ORDER BY name").fetchall())
...
def backup_full_export():
    ...
    manifest={"version":38,"created_at":now_iso(),"team":"AHVT","includes":["database","uploads"]}
...
@app.route("/backup/import/full", methods=["POST"])
def backup_full_import():
    if not has_permission("backup"): return render_template("403.html", permission="backup"),403
    f=request.files.get("full_backup_file")
    if not f or not f.filename.lower().endswith(".zip"):
        flash("اختر نسخة AHVT الكاملة بصيغة ZIP","error"); return redirect(url_for("reports_page"))
    import tempfile, zipfile as _zipfile, sqlite3 as _sqlite3
    tmp=tempfile.mkdtemp(prefix="ahvt_restore_")
    try:
        zp=os.path.join(tmp,"backup.zip"); f.save(zp)
        with _zipfile.ZipFile(zp) as z:
            if "data/ahvt.sqlite" not in z.namelist(): raise ValueError("النسخة لا تحتوي قاعدة بيانات AHVT")
            z.extractall(tmp)
        src=_sqlite3.connect(os.path.join(tmp,"data","ahvt.sqlite")); src.row_factory=_sqlite3.Row
        src_tables={r[0] for r in src.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
        live=get_db(); live_tables={r[0] for r in live.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
        if not {"settings","users"}.issubset(src_tables): raise ValueError("قاعدة البيانات غير صالحة")
        tables=sorted((src_tables & live_tables)-{"sqlite_sequence"})
        live.execute("BEGIN IMMEDIATE")
        for t in tables: live.execute(f'DELETE FROM "{t}"')
        count=0
        for t in tables:
            lc=[r[1] for r in live.execute(f'PRAGMA table_info("{t}")').fetchall()]; sc=[r[1] for r in src.execute(f'PRAGMA table_info("{t}")').fetchall()]; cols=[c for c in lc if c in sc]
            if not cols: continue
            q=','.join('"'+c+'"' for c in cols); ph=','.join('?' for _ in cols)
            for row in src.execute(f'SELECT {q} FROM "{t}"'):
                live.execute(f'INSERT INTO "{t}" ({q}) VALUES ({ph})',[row[c] for c in cols]); count+=1
        live.commit(); src.close()
        up=os.path.join(tmp,"uploads")
        if os.path.isdir(up):
            os.makedirs(UPLOAD_DIR,exist_ok=True)
            for base,_,fs in os.walk(up):
                for fn in fs:
                    srcp=os.path.join(base,fn); rel=os.path.relpath(srcp,up); dst=os.path.join(UPLOAD_DIR,rel); os.makedirs(os.path.dirname(dst),exist_ok=True); shutil.copy2(srcp,dst)
        log_action("Restored full backup",str(count)); flash("تم استرجاع النسخة الكاملة: قاعدة البيانات والملفات","ok")
    except Exception as e:
        try: get_db().rollback()
        except Exception: pass
        flash(f"تعذر الاسترجاع: {e}","error")
...
def admin_reorder():
    ...
    ids=payload.get("ids")
    table_map={"members":"members","administrators":"administrators","temporary_volunteers":"temporary_volunteers","committees":"committees"}
    table=table_map.get(entity)
    if not table or not isinstance(ids,list):
        return jsonify({"ok":False,"error":"Invalid reorder payload"}),400
    db=get_db()
    try:
        db.execute("BEGIN IMMEDIATE")
        for idx,record_id in enumerate(ids):
            db.execute(f"UPDATE {table} SET sort_order=? WHERE id=?",((idx+1)*10,str(record_id)))
        db.commit()
    except Exception as exc:
        db.rollback()
        return jsonify({"ok":False,"error":str(exc)}),500
    log_action("Reordered",entity)
    return jsonify({"ok":True,"entity":entity,"count":len(ids)})
...
def backup_import():
    ...
    try:
        ...
        table_order = [
            ...
            "tasks", "task_completions", "chat_conversations", "chat_messages", "chat_reactions", "media", "events", "certificates", "applications",
            "news", "pages", "site_sections", "nav_items", "role_permissions", "partners", "attachments",
            "card_templates", "podcast_shows", "podcast_guests", "podcast_episodes", "podcast_clips", "podcast_equipment", "temporary_volunteers", "academy_courses", "academy_lessons", "academy_quizzes", "academy_resources", "academy_enrollments", "academy_progress", "academy_payments", "finance_entries", "equipment_loans", "site_notifications",
